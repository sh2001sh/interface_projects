...
DEFAULT_CONFIG: Dict[str, Any] = {
    "version": 1,
    "server": {"host": "0.0.0.0", "port": 6109, "debug": True, "threaded": True},
    "llm": {
...
class RuntimeConfig:
    ...
    def get_server(self) -> Dict[str, Any]:
        ...
        merged = {"host": "0.0.0.0", "port": 6109, "debug": True, "threaded": True}
        if isinstance(server, dict):
            merged.update(server)
        merged["port"] = int(merged.get("port") or 6109)
        merged["debug"] = bool(merged.get("debug", True))
        merged["threaded"] = bool(merged.get("threaded", True))
        return merged
