...
class MySQLClient:
    ...
    def _get_dataset_doc_ids(self, cursor, dataset_id: str) -> List[int]:
        ...
        return self._normalize_int_list(row.get("doc_ids") if row else None)
    ...
    def _row_to_block(self, row: Dict[str, Any]) -> Block:
        ...
        if "document_split_id" in row and "block_type" not in row:
            ...
            page_range = []
            merged_pages = metadata.get("merged_pages") if isinstance(metadata, dict) else None
    ...
    def get_chunks_by_ids(self, chunk_ids: List[str], dataset_id: Optional[str] = None) -> List[Chunk]:
        ...
        if self._get_schema_mode() == "protobridge_dev":
            with self.connection() as conn:
                ...
                dataset_doc_ids = self._get_dataset_doc_ids(cursor, dataset_id) if dataset_id else []
                if dataset_doc_ids:
                    query += " AND JSON_OVERLAPS(t.doc_ids, %s)"
                    params.append(json.dumps(dataset_doc_ids, ensure_ascii=False))
    ...
    def _row_to_chunk(self, row: Dict[str, Any], dataset_id_override: Optional[str] = None) -> Chunk:
        if "task_id" in row and "content_snapshot" in row and "project_id" not in row:
            metadata = self._json_load(row.get("metadata"), {})
            if not isinstance(metadata, dict):
    ...
    def get_qa_by_id(self, qa_id: str) -> Optional[QAPair]:
        ...
        if self._get_schema_mode() == "protobridge_dev":
            with self.connection() as conn:
                ...
                self._execute(cursor, "SELECT * FROM qa_pairs WHERE qa_id = %s LIMIT 1", (qa_id,))
                fallback_row = self._fetchone_dict(cursor)
                return self._row_to_qa(fallback_row) if fallback_row else None
    ...
    def _row_to_qa(self, row: Dict[str, Any]) -> QAPair:
        if row is None:
            return None
        if "task_id" in row and "qa_id" not in row:
            validation_status = row.get("validation_result_status")
            validation_result = None
            if validation_status is not None:
                validation_result = {"passed": bool(validation_status)}
            source_chunk_ids = self._json_load(row.get("source_chunk_ids"), [])
            if not isinstance(source_chunk_ids, list):
                source_chunk_ids = []
            return QAPair(
                qa_id=str(row["id"]),
                dataset_id=str(row.get("task_id") or "") or None,
                source_block_ids=[str(item) for item in source_chunk_ids if str(item).strip()],
    ...
    def _row_to_job(self, row: Dict[str, Any]) -> FinetuneJob:
        if row is None:
            return None
        if "finetune_params" in row and "config" not in row:
            ...
            last_checkpoint_id = str(row.get("last_checkpoint_id") or "").strip()
            return FinetuneJob(
                job_id=str(row.get("job_id") or ""),
                status=status,
                base_model=row.get("base_model") or "",
                dataset_id=str(row.get("dataset_id") or ""),
                config=self._json_load(row.get("finetune_params"), {}),
                progress={
                    "current_step": row.get("current_step") or 0,
                    "total_steps": row.get("total_steps") or 0,
                    "current_epoch": float(row.get("current_epoch") or 0),
                    "monitor_url": row.get("monitor_url") or "",
                },
                last_checkpoint={"checkpoint_id": last_checkpoint_id} if last_checkpoint_id else None,
                model_path=row.get("final_model_path"),
                metrics=self._json_load(row.get("response"), {}),
                error_message=row.get("failed_msg"),
                created_at=row.get("create_time"),
                started_at=None,
                completed_at=None,
            )
