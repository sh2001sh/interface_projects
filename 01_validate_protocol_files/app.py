...
# 接口1: 协议文件校验
# POST /api/data/validate_protocol_files
...
from quick_validation import (
    DEFAULT_QUICK_SAMPLE_UNITS,
    MAX_QUICK_SAMPLE_UNITS,
    analyze_file_content_quick,
)
...
DEFAULT_VALIDATION_MODE = "quick"
DEEP_VALIDATION_MODES = {"deep", "full"}
...
def process_pdf_with_pages(
    ...
    with pdfplumber.open(file_path) as pdf:
        ...
        resolved_batch_size = max(1, int(page_batch_size or DEFAULT_PAGE_BATCH_SIZE))
...
def process_file(
    ...
    if ext == ".pdf":
        raw_blocks = process_pdf_with_pages(
            file_path,
...
def _analyze_file_content_deep(
    file_path: str,
    file_name: str,
    *,
    progress_callback: Optional[Callable[[dict], None]] = None,
    ...
    return {
        ...
        "analysis_mode": "deep",
        "sampled_units": total_pages,
        "sampled_unit_indexes": list(range(1, total_pages + 1)),
    }, issues


def _normalize_validation_mode(value: Any) -> str:
    mode = str(value or "").strip().lower()
    return mode if mode in DEEP_VALIDATION_MODES else DEFAULT_VALIDATION_MODE


def _parse_sample_unit_limit(value: Any) -> int:
    try:
        normalized = int(str(value).strip()) if value not in (None, "") else DEFAULT_QUICK_SAMPLE_UNITS
    except (TypeError, ValueError):
        normalized = DEFAULT_QUICK_SAMPLE_UNITS
...
def _analyze_file_content(
    ...
    *,
    ...
    resolved_mode = _normalize_validation_mode(validation_mode)
    if resolved_mode in DEEP_VALIDATION_MODES:
        return _analyze_file_content_deep(
            file_path,
            file_name,
            progress_callback=progress_callback,
            page_batch_size=page_batch_size,
        )
    return analyze_file_content_quick(
        file_path,
        file_name,
        progress_callback=progress_callback,
        sample_unit_limit=sample_unit_limit,
    )
    ...
    *,
    ...
    if extension_passed and size_passed and authenticity_passed:
        try:
            ...
            completeness_passed = metrics["total_units"] > 0 and metrics.get("sampled_units", metrics["total_blocks"]) > 0
            readability_message = (
                "文件内容可正常读取"
...
def _resolve_upload_split_input() -> dict[str, Any]:
    ...
    enable_llm_value = payload.get('enable_llm_postprocess') if payload else request.form.get('enable_llm_postprocess', 'true')
    enable_llm_postprocess = _as_bool(enable_llm_value, default=True)
...
def _build_upload_split_response_data(
    *,
    ...
    return {
        ...
        'protocol_field_count': protocol_field_count,
        'blocks_file_path': blocks_file_path or None,
        'doc_index': doc_index,
...
def validate_protocol_files():
    """协议文档基础校验接口。支持上传文件或本地文件路径。"""
    uploaded_files = _collect_uploaded_files()
    path_entries = _collect_input_path_entries()
    if not uploaded_files and not path_entries:
        return jsonify({
            'code': 400,
            'message': '缺少必要参数: file/files 或 file_path/file_paths'
        }), 400

    payload = _request_payload()
    max_size_raw = (payload.get('max_size_mb') if payload else request.form.get('max_size_mb', str(DEFAULT_MAX_FILE_SIZE_MB)))
    try:
        max_size_mb = int(str(max_size_raw).strip())
        if max_size_mb <= 0:
            raise ValueError
    except ValueError:
        return jsonify({
    ...
    page_batch_size = _parse_positive_int(page_batch_raw, DEFAULT_PAGE_BATCH_SIZE)
    async_value = payload.get('async') if payload else request.form.get('async')
    validation_mode_raw = payload.get('validation_mode') if payload else request.form.get('validation_mode', DEFAULT_VALIDATION_MODE)
    validation_mode = _normalize_validation_mode(validation_mode_raw)
    sample_unit_limit_raw = payload.get('sample_unit_limit') if payload else request.form.get('sample_unit_limit')
    sample_unit_limit = _parse_sample_unit_limit(sample_unit_limit_raw)

    file_entries = list(path_entries)
    file_sizes = []
    for uploaded_file in uploaded_files:
        file_sizes.append(_file_size_bytes(uploaded_file))
    for entry in path_entries:
        try:
            file_sizes.append(os.path.getsize(entry['file_path']))
        except OSError:
            file_sizes.append(0)

    async_requested = _should_async(_as_bool(async_value, default=False), file_sizes)
    if async_requested:
        saved_files = list(path_entries)
        for index, uploaded_file in enumerate(uploaded_files, start=1):
            file_path, file_name = _persist_uploaded_file(uploaded_file, prefix=f'validate_job_{index}')
            saved_files.append({'file_path': file_path, 'file_name': file_name, 'cleanup_after': True})
        job = start_job(
            'validate_protocol_files',
            lambda job_id: _run_validate_job(
                job_id,
                saved_files,
                max_size_mb,
                page_batch_size,
            ...
            metadata={
                ...
                'async_mode': True,
                'auto_triggered': not _as_bool(async_value, default=False),
                'page_batch_size': page_batch_size,
            },
        )
...
@app.route("/api/data/validate_protocol_files/status", methods=["GET"])
def validate_protocol_files_status():
...
@app.route("/api/data/validate_protocol_files/stream", methods=["GET"])
def validate_protocol_files_stream():
    job_id = str(request.args.get("job_id") or "").strip()
    if not job_id:
        return jsonify({"code": 400, "message": "缺少job_id参数", "data": None}), 400
