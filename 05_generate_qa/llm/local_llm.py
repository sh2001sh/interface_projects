...
class LocalLLM:
    ...
    def __init__(self, config: Optional[LLMConfig] = None):
        ...
        self._api_key = os.getenv("OPENAI_API_KEY") or os.getenv("openai_api_key") or ""
    ...
    def _load_model(self):
        ...
        if self.config.use_vllm:
            ...
            try:
                ...
                if self._api_key:
                    self._vllm_client.headers.update({"Authorization": f"Bearer {self._api_key}"})
                # 测试连接
                response = self._vllm_client.get(f"{self.config.vllm_url}/health")
                if response.status_code != 200:
                    raise RuntimeError(f"vLLM健康检查失败: {response.status_code}")
                ...
                raise RuntimeError(f"vLLM模型服务连接失败: {e}") from e
