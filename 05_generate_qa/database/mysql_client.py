...
class MySQLClient:
    ...
    def _get_schema_mode(self) -> str:
        ...
        try:
            ...
            has_native = all(
                self._table_exists(cursor, table_name)
                for table_name in ("blocks", "chunks", "qa_pairs", "finetune_jobs")
            )
            if has_protobridge:
    ...
    def _get_dataset_doc_ids(self, cursor, dataset_id: str) -> List[int]:
        ...
        try:
            doc_ids.append(int(resolved_dataset_id))
        except (TypeError, ValueError):
            pass

        if resolved_dataset_id.startswith("ds_"):
            try:
                doc_ids.append(int(resolved_dataset_id[3:]))
            except (TypeError, ValueError):
                pass
    ...
    def init_tables(self):
        ...
        with self.connection() as conn:
            ...
            qa_column_migrations = [
                ("dataset_id", "dataset_id TEXT" if self.is_sqlite else "dataset_id VARCHAR(64)"),
    ...
    def get_blocks_by_ids(self, block_ids: List[int], dataset_id: Optional[str] = None) -> List[Block]:
        """根据ID列表获取文档块"""
        if not block_ids:
            return []
        if self._get_schema_mode() == "protobridge_dev":
            with self.connection() as conn:
                ...
                dataset_doc_ids = self._get_dataset_doc_ids(cursor, dataset_id) if dataset_id else []
                id_column = "block_id" if dataset_doc_ids else "id"
                query = f"""
                ...
                if dataset_doc_ids:
                    query += f" AND s.document_id IN ({self._placeholders(len(dataset_doc_ids))})"
                    params.extend(dataset_doc_ids)
                    query += """
                      AND s.id = (
                          SELECT MAX(ds2.id)
                    ...
                    """
                query += " ORDER BY s.id DESC, b.id DESC"
                self._execute(cursor, query, params)
                rows = self._fetchall_dict(cursor)

                block_map: Dict[int, Block] = {}
                for row in rows:
                    block_key = int(row["block_id"] if dataset_doc_ids else row["id"])
    ...
    def get_chunks_by_ids(self, chunk_ids: List[str], dataset_id: Optional[str] = None) -> List[Chunk]:
        """根据语义块标识列表获取语义块。protobridge_dev 下优先按主键 ID 直查。"""
        if not chunk_ids:
            return []
        if self._get_schema_mode() == "protobridge_dev":
            with self.connection() as conn:
                cursor = conn.cursor()
                normalized_refs = [str(item).strip() for item in chunk_ids if str(item).strip()]
                ...
                for ref in normalized_refs:
                    if ref.isdigit():
                        numeric_ids.append(int(ref))
                    else:
                        external_chunk_ids.append(ref)
                where_clauses: List[str] = []
                params: List[Any] = []
                if numeric_ids:
                    where_clauses.append(f"m.id IN ({self._placeholders(len(numeric_ids))})")
                    params.extend(numeric_ids)
                if external_chunk_ids:
                    where_clauses.append(f"m.chunk_id IN ({self._placeholders(len(external_chunk_ids))})")
                    params.extend(external_chunk_ids)
                if not where_clauses:
                    return []
                query = f"""
                    SELECT m.*, t.id AS rag_chunk_task_id, t.doc_ids AS rag_doc_ids
                    FROM rag_chunk_metadata m
                    JOIN rag_chunk_task t ON t.id = m.task_id
                    WHERE ({' OR '.join(where_clauses)})
                      AND COALESCE(m.deleted, 0) = 0
                      AND COALESCE(t.deleted, 0) = 0
                """
                query += " ORDER BY m.task_id DESC, m.id DESC"
                self._execute(cursor, query, params)
                rows = self._fetchall_dict(cursor)

                chunk_map: Dict[str, Chunk] = {}
                for row in rows:
                    chunk = self._row_to_chunk(row, dataset_id_override=dataset_id)
                    chunk_key = str(row.get("chunk_id") or "").strip()
                    primary_key = str(row.get("id") or "").strip()
                    if primary_key and primary_key not in chunk_map:
                        chunk_map[primary_key] = chunk
                    if chunk_key and chunk_key not in chunk_map:
                        chunk_map[chunk_key] = chunk
                return [chunk_map[chunk_id] for chunk_id in normalized_refs if chunk_id in chunk_map]

        with self.connection() as conn:
            cursor = conn.cursor()
            placeholders = self._placeholders(len(chunk_ids))
            self._execute(
                cursor,
            ...
            rows = self._fetchall_dict(cursor)
            chunk_map = {str(row["chunk_id"]): self._row_to_chunk(row) for row in rows}
    ...
    def get_chunks_by_dataset(self, dataset_id: str) -> List[Chunk]:
        ...
        if self._get_schema_mode() == "protobridge_dev":
            with self.connection() as conn:
                cursor = conn.cursor()
                dataset_doc_ids = self._get_dataset_doc_ids(cursor, dataset_id)
                    ...
                    self._execute(
                        ...
                        """
                            ...
                            WHERE COALESCE(m.deleted, 0) = 0
                              AND COALESCE(t.deleted, 0) = 0
                              AND JSON_OVERLAPS(t.doc_ids, %s)
                            ORDER BY m.task_id DESC, m.id DESC
                        """,
                        (json.dumps(dataset_doc_ids, ensure_ascii=False),),
                    )
                    rows = self._fetchall_dict(cursor)
                if not rows:
                    self._execute(
                ...
                for row in rows:
                    ...
                    if chunk_key not in chunk_map:
                        chunk_map[chunk_key] = self._row_to_chunk(row, dataset_id_override=dataset_id)
    ...
    def get_chunks_by_datasets(self, dataset_ids: List[str]) -> List[Chunk]:
        """根据多个数据集ID获取语义块，并按主键/业务ID去重。"""
        ...
        for dataset_id in normalized_ids:
            for chunk in self.get_chunks_by_dataset(dataset_id):
                chunk_key = str(
                    (chunk.metadata or {}).get("chunk_record_id")
                    or (chunk.metadata or {}).get("id")
                    or chunk.chunk_id
                ).strip()
                if not chunk_key or chunk_key in seen_chunks:
                    continue
                seen_chunks.add(chunk_key)
                if not chunk.dataset_id:
                    chunk.dataset_id = dataset_id
                merged_chunks.append(chunk)
        return merged_chunks

    def _row_to_chunk(self, row: Dict[str, Any], dataset_id_override: Optional[str] = None) -> Chunk:
        if (
            "task_id" in row
            and "project_id" not in row
            and ("content_snapshot" in row or "merged_content" in row)
        ):
            metadata = self._json_load(row.get("metadata"), {})
            if not isinstance(metadata, dict):
                metadata = {"metadata": metadata}
            metadata.setdefault("chunk_record_id", row.get("id"))
            metadata.setdefault("task_id", row.get("task_id"))
            metadata.setdefault("milvus_pk", row.get("milvus_pk"))
            merged_content = row.get("merged_content")
            if isinstance(merged_content, str) and merged_content.strip():
                metadata.setdefault("merged_content", merged_content)
            content_snapshot = row.get("content_snapshot")
            if not isinstance(content_snapshot, str) or not content_snapshot.strip():
                content_snapshot = merged_content or ""
            return Chunk(
                chunk_id=str(row["chunk_id"]),
                project_id=str(metadata.get("project_id") or ""),
                dataset_id=str(dataset_id_override or row.get("task_id") or ""),
                source_block_ids=self._normalize_int_list(row.get("source_block_ids")),
    ...
    def insert_qa(self, qa: QAPair) -> str:
        ...
        with self.connection() as conn:
            ...
            self._execute(
                cursor,
                f"""
                    INSERT INTO qa_pairs (qa_id, dataset_id, source_block_ids, question, answer,
    ...
    def get_qa_by_dataset_and_id(self, dataset_id: str, qa_id: str) -> Optional[QAPair]:
        """根据 dataset_id 和 qa_id 获取 QA 对。"""
        with self.connection() as conn:
            cursor = conn.cursor()
            self._execute(
                cursor,
                "SELECT * FROM qa_pairs WHERE dataset_id = %s AND qa_id = %s LIMIT 1",
                (dataset_id, qa_id),
            )
            row = self._fetchone_dict(cursor)
            return self._row_to_qa(row) if row else None
