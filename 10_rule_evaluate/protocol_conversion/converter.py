...
def _normalize_c_style_boolean_ops(text: str) -> str:
    normalized = str(text or "").strip()
    if not normalized:
        return normalized
...
def _normalize_formula_expression_syntax(formula: str) -> str:
    normalized = _clean_formula_text(formula)
    if not normalized:
        return normalized
    ...
    if ternary_parts is not None:
        ...
        rendered_condition = _normalize_c_style_boolean_ops(
            _normalize_formula_expression_syntax(condition)
        )
        rendered_true = _normalize_formula_expression_syntax(when_true)
        rendered_false = _normalize_formula_expression_syntax(when_false)
        return f"({rendered_true} if {rendered_condition} else {rendered_false})"
    return _normalize_c_style_boolean_ops(normalized)
...
def _looks_like_expression_formula(formula: str) -> bool:
    text = _normalize_formula_expression_syntax(formula)
...
def extract_executable_formula(text: str, mode: str, source_field: str = "") -> str:
    ...
    for candidate in _extract_transcoding_candidates(cleaned, source_field=source_field):
        if _looks_like_executable_formula(candidate):
            return _normalize_formula_expression_syntax(candidate)
    if _looks_like_executable_formula(cleaned):
        return _normalize_formula_expression_syntax(cleaned)
    return ""
...
def _build_rule(item: Dict[str, Any], source: str = "llm") -> Optional[ConversionRule]:
    ...
    if conversion_mode != "mapping" and not _looks_like_block_formula(formula):
...
def execute_protocol_conversion(
    ...
    return {
        ...
        "embedding_model": knowledge_base.embedding_model if knowledge_base is not None else None,
