from __future__ import annotations

import json
import math
import re
import uuid
import xml.etree.ElementTree as ET
from concurrent.futures import ThreadPoolExecutor, as_completed
from dataclasses import dataclass
from difflib import SequenceMatcher
from pathlib import Path
from typing import Any, Callable, Dict, Iterable, List, Optional, Sequence, Tuple

import config

from shared.model_metadata import resolve_model_metadata
from .evaluation import (
    _cosine_similarity,
    _fallback_similarity,
    _load_backends,
    _normalize_cosine,
    _rerank_similarity,
)


@dataclass
class RuleFieldSpec:
    protocol_name: str
    file_name: str
    field_name: str
    normalized_name: str
    path: str
    path_parts: Tuple[str, ...]
    bit_length: Optional[int]
    default_value: Optional[str]
    source_tag: str
    semantic_group: Optional[str]


SEMANTIC_GROUP_PATTERNS: Dict[str, Tuple[str, ...]] = {
    "longitude": ("经度", "longitude", "lon"),
    "latitude": ("纬度", "latitude", "lat"),
    "altitude": ("高度", "高程", "海拔", "altitude", "height", "elevation"),
    "pitch": ("俯仰", "pitch"),
    "roll": ("翻滚", "横滚", "roll"),
    "yaw": ("偏航", "航向", "yaw", "heading"),
    "speed": ("速度", "speed", "velocity"),
    "direction": ("方向", "bearing", "course"),
    "time": ("时间", "时刻", "飞临时间", "time"),
    "day": ("日", "day"),
    "hour": ("小时", "hour"),
    "minute": ("分钟", "minute", "min"),
    "second": ("秒", "second", "sec"),
    "threat_type": ("威胁类型",),
    "threat_form": ("威胁形式",),
    "target_id": ("目标编号", "targetid"),
    "unit_id": ("作战单元编号", "unitid"),
    "quantity": ("数量", "目标数量", "quantity", "count"),
    "name": ("名称", "name"),
    "info": ("信息", "info"),
    "fpi": ("fpi",),
    "gpi": ("gpi",),
}

DIRECT_RULE_TYPES = {"direct", "transcoding", "mapping"}
EXPR_RULE_TYPES = {"expression", "python_expr", "python_block", "formula"}
CONST_RULE_TYPES = {"const", "constant"}
ALLOWED_RULE_TYPES = DIRECT_RULE_TYPES | EXPR_RULE_TYPES | CONST_RULE_TYPES
RULE_REF_PATTERN = re.compile(r"[A-Za-z_][A-Za-z0-9_]*\.[^\s\+\-\*/<>=!\(\),?:]+")
NUMERIC_LITERAL_PATTERN = re.compile(r"^[+-]?(?:\d+(?:\.\d+)?|\.\d+)$")


def _local_name(tag: str) -> str:
    if "}" in tag:
        return tag.split("}", 1)[1]
    if ":" in tag:
        return tag.split(":", 1)[1]
    return tag


def _parse_int(raw: Optional[str]) -> Optional[int]:
    text = str(raw or "").strip()
    if not text:
        return None
    try:
        return int(text)
    except ValueError:
        return None


def _normalize_lookup_value(value: Any) -> str:
    text = str(value or "").strip().lower()
    text = re.sub(r"[\s_\-./:：，,()\[\]{}]+", "", text)
    return text


def _iter_source_field_alias_keys(value: Any) -> List[str]:
    text = str(value or "").strip()
    if not text:
        return []
    candidates: List[str] = [text]
    if "/" in text:
        candidates.append(text.split("/")[-1].strip())
    if "." in text:
        candidates.append(text.split(".")[-1].strip())
    underscore_parts = [part.strip() for part in text.split("_") if part.strip()]
    for index in range(1, len(underscore_parts)):
        candidates.append("_".join(underscore_parts[index:]))
        candidates.append(underscore_parts[index])
    normalized: List[str] = []
    seen = set()
    for candidate in candidates:
        key = _normalize_lookup_value(candidate)
        if not key or key in seen:
            continue
        seen.add(key)
        normalized.append(key)
    return normalized


def _infer_semantic_group(text: str) -> Optional[str]:
    normalized = _normalize_lookup_value(text)
    if not normalized:
        return None
    for group, patterns in SEMANTIC_GROUP_PATTERNS.items():
        if any(_normalize_lookup_value(pattern) in normalized for pattern in patterns):
            return group
    return None


def _field_text(spec: RuleFieldSpec) -> str:
    parts = [
        f"字段名:{spec.field_name}",
        f"路径:{spec.path}",
        f"协议:{spec.protocol_name}",
        f"文件:{spec.file_name}",
    ]
    if spec.bit_length is not None:
        parts.append(f"位宽:{spec.bit_length}")
    if spec.default_value is not None:
        parts.append(f"默认值:{spec.default_value}")
    if spec.semantic_group:
        parts.append(f"语义组:{spec.semantic_group}")
    return " | ".join(parts)


def _default_export_root() -> Path:
    runtime_root = Path(config.DATA_DIR).resolve().parent
    return runtime_root / "deliverables" / "milvus_exports"


def _sanitize_collection_name(value: Any) -> str:
    text = re.sub(r"[^0-9A-Za-z_]+", "_", str(value or "").strip())
    return text.strip("_") or "rule_evaluate_embeddings"


def _build_export_record_id(trace_id: str, kind: str, protocol_name: str, normalized_name: str) -> str:
    return f"{trace_id}:{kind}:{protocol_name}:{normalized_name}"


def _build_milvus_export_record(
    *,
    trace_id: str,
    collection_name: str,
    kind: str,
    spec: RuleFieldSpec,
    content: str,
    embedding: Sequence[float],
) -> Dict[str, Any]:
    return {
        "id": _build_export_record_id(trace_id, kind, spec.protocol_name, spec.normalized_name),
        "chunk_id": f"{kind}:{spec.protocol_name}:{spec.normalized_name}",
        "project_id": config.PROJECT_NAME,
        "dataset_id": trace_id,
        "semantic_type": kind,
        "content": content,
        "embedding": [float(value) for value in embedding],
    }


def _write_milvus_export_file(
    *,
    trace_id: str,
    collection_name: str,
    embedding_dim: int,
    embedding_model_meta: Dict[str, Any],
    records: Sequence[Dict[str, Any]],
) -> str:
    export_root = _default_export_root()
    export_root.mkdir(parents=True, exist_ok=True)
    export_path = export_root / f"{trace_id}_milvus_import.json"
    payload = {
        "format": "milvus_insert_rows_v1",
        "collection_name": collection_name,
        "embedding_dim": int(embedding_dim),
        "trace_id": trace_id,
        "project_name": config.PROJECT_NAME,
        "embedding_model_meta": embedding_model_meta,
        "row_count": len(records),
        "row_schema_fields": [
            "id",
            "chunk_id",
            "project_id",
            "dataset_id",
            "semantic_type",
            "content",
            "embedding",
        ],
        "rows": list(records),
    }
    export_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return str(export_path)


def _encode_specs_for_export(
    *,
    embedding: Any,
    specs: Sequence[RuleFieldSpec],
    trace_id: str,
    collection_name: str,
    kind: str,
    batch_size: int,
) -> List[Dict[str, Any]]:
    records: List[Dict[str, Any]] = []
    normalized_batch_size = max(1, int(batch_size or 1))
    for start in range(0, len(specs), normalized_batch_size):
        batch_specs = specs[start : start + normalized_batch_size]
        batch_texts = [_field_text(spec) for spec in batch_specs]
        batch_vectors = embedding.encode(batch_texts)
        for index, spec in enumerate(batch_specs):
            vector = batch_vectors[index]
            if hasattr(vector, "tolist"):
                vector = vector.tolist()
            records.append(
                _build_milvus_export_record(
                    trace_id=trace_id,
                    collection_name=collection_name,
                    kind=kind,
                    spec=spec,
                    content=batch_texts[index],
                    embedding=vector,
                )
            )
    return records


def _append_field_specs(
    node: ET.Element,
    protocol_name: str,
    file_name: str,
    path_parts: Tuple[str, ...],
    output: List[RuleFieldSpec],
) -> None:
    local = _local_name(node.tag)
    node_name = str(node.attrib.get("name") or "").strip() or local
    current_path = path_parts + ((node_name,) if node_name else ())

    if local in {"Item", "StructMess"}:
        field_name = node_name
        normalized_name = _normalize_lookup_value(field_name)
        if normalized_name:
            output.append(
                RuleFieldSpec(
                    protocol_name=protocol_name,
                    file_name=file_name,
                    field_name=field_name,
                    normalized_name=normalized_name,
                    path="/".join(current_path),
                    path_parts=current_path,
                    bit_length=_parse_int((node.text or "").strip()) or _parse_int(node.attrib.get("bitLength")) or _parse_int(node.attrib.get("length")),
                    default_value=str(node.attrib.get("defaultValue") or "").strip() or None,
                    source_tag=local,
                    semantic_group=_infer_semantic_group(field_name),
                )
            )
        return

    next_path = current_path if local in {"Field", "Group", "NameSpace"} else path_parts
    for child in list(node):
        _append_field_specs(child, protocol_name, file_name, next_path, output)


def _iter_protocol_scope_names(values: Any) -> Iterable[str]:
    if isinstance(values, (list, tuple, set)):
        for item in values:
            text = str(item or "").strip()
            if text:
                yield text
        return
    text = str(values or "").strip()
    if text:
        yield text


def _unwrap_rule_payload(payload: Any) -> Any:
    current = payload
    while isinstance(current, dict):
        nested = current.get("data")
        if isinstance(nested, dict):
            current = nested
            continue
        nested = current.get("body")
        if isinstance(nested, dict):
            current = nested
            continue
        break
    return current


def _extract_protocol_scope(payload: Any) -> Tuple[List[str], List[str]]:
    current = _unwrap_rule_payload(payload)
    source_protocols: List[str] = []
    target_protocols: List[str] = []
    seen_sources = set()
    seen_targets = set()

    def append_source(value: Any) -> None:
        for item in _iter_protocol_scope_names(value):
            normalized = _normalize_lookup_value(item)
            if not normalized or normalized in seen_sources:
                continue
            seen_sources.add(normalized)
            source_protocols.append(item)

    def append_target(value: Any) -> None:
        for item in _iter_protocol_scope_names(value):
            normalized = _normalize_lookup_value(item)
            if not normalized or normalized in seen_targets:
                continue
            seen_targets.add(normalized)
            target_protocols.append(item)

    if isinstance(current, dict):
        relations = current.get("relations")
        if isinstance(relations, list):
            for relation in relations:
                if not isinstance(relation, dict):
                    continue
                append_source(relation.get("source_protocols"))
                append_source(relation.get("source_protocol"))
                append_target(relation.get("target_protocol"))
        conversions = current.get("conversions")
        if isinstance(conversions, list):
            for conversion in conversions:
                if not isinstance(conversion, dict):
                    continue
                sources = conversion.get("sources")
                if isinstance(sources, list):
                    for source in sources:
                        if isinstance(source, dict):
                            append_source(source.get("protocol"))
                        else:
                            append_source(source)
                target_payload = conversion.get("target")
                if isinstance(target_payload, dict):
                    append_target(target_payload.get("protocol"))
                append_target(conversion.get("target_protocol"))
    return source_protocols, target_protocols


def _load_protocol_fields_from_dir(
    path_like: Any,
    field_name: str,
    allowed_protocols: Optional[Sequence[str]] = None,
) -> List[RuleFieldSpec]:
    if isinstance(path_like, (list, tuple)):
        dirs = [Path(str(item or "").strip()) for item in path_like]
    else:
        dirs = [Path(str(path_like or "").strip())]
    if not dirs or any(not str(item) for item in dirs):
        raise ValueError(f"{field_name}不能为空")

    normalized_allowed = {
        _normalize_lookup_value(item)
        for item in (allowed_protocols or [])
        if _normalize_lookup_value(item)
    }
    results: List[RuleFieldSpec] = []
    available_protocols: List[str] = []
    for index, directory in enumerate(dirs):
        label = field_name if len(dirs) == 1 else f"{field_name}[{index}]"
        if not directory.exists() or not directory.is_dir():
            raise ValueError(f"{label}不存在: {directory}")
        xml_files = sorted(directory.glob("*.xml"))
        if not xml_files:
            raise ValueError(f"{label}下未找到 XML 文件: {directory}")
        for xml_file in xml_files:
            available_protocols.append(xml_file.stem)
            if normalized_allowed and _normalize_lookup_value(xml_file.stem) not in normalized_allowed:
                continue
            tree = ET.parse(xml_file)
            root = tree.getroot()
            protocol_name = xml_file.stem
            _append_field_specs(root, protocol_name, xml_file.name, tuple(), results)
    if normalized_allowed and not results:
        raise ValueError(
            f"{field_name}中未找到规则文件指定的协议: requested={list(allowed_protocols or [])}, "
            f"available={available_protocols}"
        )
    return results


def _load_json_like(value: Any, field_name: str) -> Any:
    if value is None:
        raise ValueError(f"{field_name}不能为空")
    if isinstance(value, (dict, list)):
        return value
    raw = str(value).strip()
    if not raw:
        raise ValueError(f"{field_name}不能为空")
    candidate = Path(raw)
    if candidate.exists() and candidate.is_file():
        return json.loads(candidate.read_text(encoding="utf-8-sig"))
    return json.loads(raw)


def _extract_rule_items(payload: Any) -> List[Dict[str, Any]]:
    if isinstance(payload, dict) and isinstance(payload.get("data"), dict):
        return _extract_rule_items(payload["data"])
    if isinstance(payload, dict) and isinstance(payload.get("body"), dict):
        return _extract_rule_items(payload["body"])
    if isinstance(payload, dict):
        conversions = payload.get("conversions")
        if isinstance(conversions, list):
            items: List[Dict[str, Any]] = []
            for conversion in conversions:
                if not isinstance(conversion, dict):
                    continue
                rules = conversion.get("rules")
                if isinstance(rules, list):
                    items.extend(item for item in rules if isinstance(item, dict))
            if items:
                return items
        relations = payload.get("relations")
        if isinstance(relations, list):
            items = []
            for relation in relations:
                if not isinstance(relation, dict):
                    continue
                rules = relation.get("rules")
                if isinstance(rules, list):
                    items.extend(item for item in rules if isinstance(item, dict))
            if items:
                return items
        for key in ("rules", "generated_rules", "target_field_rules", "normalized_rules"):
            value = payload.get(key)
            if isinstance(value, list):
                return [item for item in value if isinstance(item, dict)]
    if isinstance(payload, list):
        return [item for item in payload if isinstance(item, dict)]
    return []


def _normalize_source_fields(item: Dict[str, Any]) -> List[str]:
    preferred_values: List[str] = []
    for value in item.get("source_paths") or []:
        text = str(value).strip()
        if not text:
            continue
        preferred_values.append(text)
        preferred_values.append(text.split("/")[-1].strip())
    for binding in item.get("source_bindings") or []:
        if not isinstance(binding, dict):
            continue
        for key in ("display_field", "field_name", "source_path", "actual_field"):
            text = str(binding.get(key) or "").strip()
            if text:
                preferred_values.append(text)
                if key == "source_path":
                    preferred_values.append(text.split("/")[-1].strip())
    for value in item.get("source_actual_fields") or []:
        text = str(value).strip()
        if text:
            preferred_values.append(text)
    if preferred_values:
        deduped_preferred: List[str] = []
        seen_preferred = set()
        for value in preferred_values:
            if value in seen_preferred:
                continue
            seen_preferred.add(value)
            deduped_preferred.append(value)
        return deduped_preferred

    raw = item.get("source_fields")
    values: List[str] = []
    if isinstance(raw, list):
        values = [str(value).strip() for value in raw if str(value).strip()]
    elif isinstance(raw, str):
        values = [value.strip() for value in raw.split(",") if value.strip()]
    fallback = str(item.get("field_name") or item.get("source_field") or "").strip()
    if not values and fallback:
        values = [fallback]
    return values


def _iter_rule_target_keys(item: Dict[str, Any]) -> List[str]:
    keys: List[str] = []
    seen = set()
    for raw_value in (
        item.get("target_actual_field"),
        item.get("target_path"),
        item.get("target_field"),
        item.get("field_name"),
    ):
        text = str(raw_value or "").strip()
        if not text:
            continue
        candidates = [text]
        if "/" in text:
            candidates.append(text.split("/")[-1].strip())
        if "." in text:
            candidates.append(text.split(".")[-1].strip())
        for candidate in candidates:
            key = _normalize_lookup_value(candidate)
            if not key or key in seen:
                continue
            seen.add(key)
            keys.append(key)
    return keys


def _is_zero_fallback_rule(item: Optional[Dict[str, Any]]) -> bool:
    if not isinstance(item, dict):
        return False
    formula = str(item.get("formula") or item.get("rule") or "").strip()
    source_fields = [str(value).strip() for value in (item.get("source_fields") or []) if str(value).strip()]
    rule_type = str(item.get("rule_type") or "").strip().lower()
    return formula in {"0", "0.0", "0U", "0L"} and not source_fields and rule_type in CONST_RULE_TYPES


def _duplicate_target_actual_key(item: Dict[str, Any]) -> Optional[str]:
    actual_field = str(item.get("target_actual_field") or "").strip()
    if not actual_field:
        return None
    normalized = _normalize_lookup_value(actual_field)
    return normalized or None


def _infer_rule_type(item: Dict[str, Any], formula: str, source_fields: List[str]) -> str:
    raw_type = str(item.get("rule_type") or item.get("formula_kind") or item.get("conversion_mode") or "").strip().lower()
    if raw_type in ALLOWED_RULE_TYPES:
        return raw_type
    normalized_formula = formula.strip()
    if not source_fields and NUMERIC_LITERAL_PATTERN.fullmatch(normalized_formula):
        return "const"
    if normalized_formula == "0":
        return "const"
    if len(source_fields) == 1:
        cleaned = source_fields[0].split(".")[-1].strip()
        if cleaned and cleaned in formula:
            return "direct"
    return "expression"


def _normalize_rule_items(payload: Any) -> List[Dict[str, Any]]:
    rules: List[Dict[str, Any]] = []
    for item in _extract_rule_items(payload):
        target_field = str(item.get("target_field") or item.get("field_name") or "").strip()
        formula = str(item.get("formula") or item.get("rule") or item.get("conversion_formula") or "").strip()
        source_fields = _normalize_source_fields(item)
        if not target_field:
            continue
        rules.append(
            {
                "target_field": target_field,
                "target_actual_field": str(item.get("target_actual_field") or "").strip() or None,
                "target_path": str(item.get("target_path") or "").strip() or None,
                "formula": formula,
                "source_fields": source_fields,
                "rule_type": _infer_rule_type(item, formula, source_fields),
                "default_value": item.get("default_value"),
                "description": item.get("description"),
                "when": item.get("when"),
            }
        )
    return rules


def _build_field_indices(fields: Sequence[RuleFieldSpec]) -> Tuple[Dict[str, RuleFieldSpec], Dict[str, List[RuleFieldSpec]]]:
    by_name: Dict[str, RuleFieldSpec] = {}
    alias_index: Dict[str, List[RuleFieldSpec]] = {}
    for field in fields:
        by_name.setdefault(field.normalized_name, field)
        for alias in {
            field.normalized_name,
            _normalize_lookup_value(field.field_name),
            _normalize_lookup_value(field.path_parts[-1] if field.path_parts else field.field_name),
        }:
            if not alias:
                continue
            bucket = alias_index.setdefault(alias, [])
            if field not in bucket:
                bucket.append(field)
    return by_name, alias_index


def _resolve_rule_field_refs(field_names: Sequence[str], alias_index: Dict[str, List[RuleFieldSpec]]) -> List[RuleFieldSpec]:
    results: List[RuleFieldSpec] = []
    seen = set()
    for field_name in field_names:
        for key in _iter_source_field_alias_keys(field_name):
            for candidate in alias_index.get(key, []):
                signature = (candidate.protocol_name, candidate.normalized_name, candidate.path)
                if signature in seen:
                    continue
                seen.add(signature)
                results.append(candidate)
    return results


def _score_text_similarity(query_text: str, doc_text: str, backend: Any) -> float:
    if backend.embedding is not None:
        return _normalize_cosine(_cosine_similarity(backend.embedding, query_text, doc_text))
    return _fallback_similarity(query_text, doc_text)


def _semantic_conflict(target_group: Optional[str], source_group: Optional[str]) -> bool:
    if not target_group or not source_group:
        return False
    directional = {"longitude", "latitude", "altitude", "pitch", "roll", "yaw", "speed", "direction"}
    if target_group in directional and source_group in directional and target_group != source_group:
        return True
    if target_group in {"threat_type", "threat_form"} and source_group in {"threat_type", "threat_form"} and target_group != source_group:
        return True
    return False


def _score_dimension_consistency(target_field: RuleFieldSpec, source_fields: Sequence[RuleFieldSpec], rule_type: str) -> Tuple[float, bool]:
    if rule_type in CONST_RULE_TYPES or not source_fields:
        return 0.0, False
    if any(_semantic_conflict(target_field.semantic_group, source.semantic_group) for source in source_fields):
        return 0.0, False
    if target_field.semantic_group and any(source.semantic_group == target_field.semantic_group for source in source_fields):
        return 100.0, True
    if target_field.bit_length and any(source.bit_length == target_field.bit_length for source in source_fields if source.bit_length):
        return 80.0, True
    if rule_type in EXPR_RULE_TYPES and len(source_fields) > 1:
        return 70.0, True
    if not target_field.semantic_group and all(source.semantic_group is None for source in source_fields):
        return 60.0, True
    return 35.0, False


def _average_candidate_similarity(items: Sequence[Dict[str, Any]], key: str) -> float:
    if not items:
        return 0.0
    normalized = [_normalize_similarity_score(item.get(key, 0.0)) for item in items]
    return round(sum(normalized) / max(len(normalized), 1) * 100.0, 4)


def _normalize_similarity_score(value: Any) -> float:
    try:
        score = float(value)
    except Exception:
        return 0.0
    if math.isnan(score) or math.isinf(score):
        return 0.0
    if score > 1.0 and score <= 100.0:
        score /= 100.0
    return max(0.0, min(1.0, score))


def _clamp_percent(value: Any) -> float:
    try:
        score = float(value)
    except Exception:
        return 0.0
    if math.isnan(score) or math.isinf(score):
        return 0.0
    return round(max(0.0, min(100.0, score)), 4)


def _formula_is_valid(formula: str, rule_type: str) -> bool:
    text = str(formula or "").strip()
    if not text:
        return False
    if rule_type in CONST_RULE_TYPES:
        return bool(NUMERIC_LITERAL_PATTERN.fullmatch(text))
    if text.count("(") != text.count(")"):
        return False
    if NUMERIC_LITERAL_PATTERN.fullmatch(text):
        return True
    if rule_type in DIRECT_RULE_TYPES:
        return bool(RULE_REF_PATTERN.search(text) or re.search(r"[A-Za-z0-9_\u4e00-\u9fff]+", text))
    return len(text) >= 2


def _safe_percent(numerator: float, denominator: float) -> float:
    if denominator <= 0:
        return 0.0
    return _clamp_percent(numerator / denominator * 100.0)


def _emit_progress(
    callback: Optional[Callable[[Dict[str, Any]], None]],
    payload: Dict[str, Any],
) -> None:
    if callback is None:
        return
    callback(payload)


def evaluate_protocol_rules(
    source_protocol_dirs: Any,
    target_protocol_dir: Any,
    conversion_rules: Any,
    coarse_top_k: int = 10,
    coarse_similarity_threshold: float = 0.55,
    fine_similarity_threshold: float = 0.75,
    use_model_inference: bool = True,
    allow_modelscope_download: bool = True,
    trace_id: Optional[str] = None,
    batch_size: int = 8,
    max_workers: int = 1,
    export_payload: bool = False,
    export_name: Optional[str] = None,
    progress_callback: Optional[Callable[[Dict[str, Any]], None]] = None,
) -> Dict[str, Any]:
    rules_payload = _load_json_like(conversion_rules, "conversion_rules")
    scoped_source_protocols, scoped_target_protocols = _extract_protocol_scope(rules_payload)
    source_fields = _load_protocol_fields_from_dir(
        source_protocol_dirs,
        "source_protocol_dirs",
        allowed_protocols=scoped_source_protocols or None,
    )
    target_fields = _load_protocol_fields_from_dir(
        target_protocol_dir,
        "target_protocol_dir",
        allowed_protocols=scoped_target_protocols or None,
    )
    rules = _normalize_rule_items(rules_payload)
    _emit_progress(
        progress_callback,
        {
            "stage": "loading_models",
            "message": "规则评估已开始，正在加载模型与协议字段",
            "source_field_count": len(source_fields),
            "target_field_count": len(target_fields),
            "rule_count": len(rules),
        },
    )
    backend = _load_backends(use_model_inference=use_model_inference, allow_modelscope_download=allow_modelscope_download)
    embedding_model_meta = resolve_model_metadata(
        model_name=config.EMBED_MODEL_NAME,
        model_dir=backend.embedding_info.model_dir,
        model_cache_dir=config.MODEL_CACHE_DIR,
    )
    reranker_model_meta = resolve_model_metadata(
        model_name=config.RERANK_MODEL_NAME,
        model_dir=backend.reranker_info.model_dir,
        model_cache_dir=config.MODEL_CACHE_DIR,
    )
    trace = str(trace_id or uuid.uuid4())
    normalized_batch_size = max(1, int(batch_size or 1))
    normalized_max_workers = max(1, int(max_workers or 1))
    export_path: Optional[str] = None

    if export_payload:
        if backend.embedding is None:
            raise ValueError("export_payload=true 时必须启用真实 embedding 模型推理")
        export_collection_name = _sanitize_collection_name(
            export_name or f"{config.PROJECT_NAME}_rule_evaluate_embeddings"
        )
        _emit_progress(
            progress_callback,
            {
                "stage": "exporting_milvus_payload",
                "message": "正在生成 Milvus 导入文件",
                "source_field_count": len(source_fields),
                "target_field_count": len(target_fields),
            },
        )
        source_records = _encode_specs_for_export(
            embedding=backend.embedding,
            specs=source_fields,
            trace_id=trace,
            collection_name=export_collection_name,
            kind="source_field",
            batch_size=normalized_batch_size,
        )
        target_records = _encode_specs_for_export(
            embedding=backend.embedding,
            specs=target_fields,
            trace_id=trace,
            collection_name=export_collection_name,
            kind="target_field",
            batch_size=normalized_batch_size,
        )
        export_records = source_records + target_records
        embedding_dim = len(export_records[0]["embedding"]) if export_records else 0
        export_path = _write_milvus_export_file(
            trace_id=trace,
            collection_name=export_collection_name,
            embedding_dim=embedding_dim,
            embedding_model_meta=embedding_model_meta,
            records=export_records,
        )

    _, source_alias_index = _build_field_indices(source_fields)
    target_index, target_alias_index = _build_field_indices(target_fields)
    rule_map: Dict[str, Dict[str, Any]] = {}
    duplicate_targets = set()
    actual_target_counter: Dict[str, int] = {}
    for rule in rules:
        local_keys = _iter_rule_target_keys(rule)
        rule["_target_match_keys"] = local_keys
        actual_key = _duplicate_target_actual_key(rule)
        if actual_key:
            actual_target_counter[actual_key] = actual_target_counter.get(actual_key, 0) + 1
        for normalized_target in local_keys:
            rule_map.setdefault(normalized_target, rule)
    duplicate_targets = {key for key, count in actual_target_counter.items() if count > 1}

    field_results: List[Dict[str, Any]] = []
    coarse_recall_hit_count = 0
    fine_accept_count = 0
    non_zero_rule_count = 0
    zero_fallback_count = 0
    covered_field_count = 0
    converted_field_count = 0
    invalid_rule_count = 0
    field_match_values: List[float] = []
    semantic_values: List[float] = []
    dimension_values: List[float] = []
    structure_values: List[float] = []
    convertible_field_match_values: List[float] = []
    convertible_semantic_values: List[float] = []
    convertible_dimension_values: List[float] = []
    convertible_structure_values: List[float] = []
    successful_field_match_values: List[float] = []
    successful_semantic_values: List[float] = []
    successful_dimension_values: List[float] = []
    successful_structure_values: List[float] = []
    total_batches = max(1, math.ceil(len(target_fields) / normalized_batch_size))
    processed_targets = 0

    def evaluate_target_field(target_field: RuleFieldSpec) -> Dict[str, Any]:
        target_key = target_field.normalized_name
        rule = rule_map.get(target_key)
        target_text = _field_text(target_field)

        scored_candidates: List[Dict[str, Any]] = []
        for source_field in source_fields:
            similarity = _score_text_similarity(target_text, _field_text(source_field), backend)
            if similarity < float(coarse_similarity_threshold):
                continue
            scored_candidates.append(
                {
                    "field_name": source_field.field_name,
                    "protocol_name": source_field.protocol_name,
                    "path": source_field.path,
                    "semantic_group": source_field.semantic_group,
                    "coarse_similarity": round(_normalize_similarity_score(similarity), 4),
                    "_spec": source_field,
                    "content": _field_text(source_field),
                }
            )
        scored_candidates.sort(key=lambda item: item["coarse_similarity"], reverse=True)
        top_candidates = scored_candidates[: max(int(coarse_top_k), 1)]
        coarse_hit = bool(top_candidates)

        fine_candidates = []
        if top_candidates:
            if backend.reranker is not None:
                reranked = backend.reranker.rerank_pairs(target_text, top_candidates, top_k=len(top_candidates), text_key="content")
                for item in reranked:
                    probability = item.get("rerank_probability")
                    if probability is not None:
                        item["fine_similarity"] = round(_normalize_similarity_score(probability), 4)
                    else:
                        item["fine_similarity"] = round(_normalize_similarity_score(1.0 / (1.0 + math.exp(-float(item.get("rerank_score", 0.0))))), 4)
                    fine_candidates.append(item)
            else:
                for item in top_candidates:
                    item = dict(item)
                    item["fine_similarity"] = item["coarse_similarity"]
                    fine_candidates.append(item)
        accepted_candidates = [item for item in fine_candidates if float(item.get("fine_similarity", 0.0)) >= float(fine_similarity_threshold)]

        issues: List[str] = []
        status = "missing_rule"
        rule_type = None
        formula = None
        source_match_specs: List[RuleFieldSpec] = []
        field_match_score = 0.0
        semantic_score = 0.0
        dimension_score = 0.0
        structure_score = 100.0
        structure_pass = True
        formula_valid = False

        if rule:
            rule_type = str(rule["rule_type"]).strip().lower()
            formula = str(rule["formula"]).strip()
            if rule_type in CONST_RULE_TYPES and formula == "0":
                status = "fallback_zero"
                field_match_score = 0.0
                semantic_score = 0.0
                dimension_score = 0.0
                structure_score = 0.0
                structure_pass = False
            if target_key in duplicate_targets:
                issues.append("目标字段存在重复规则")
                structure_score -= 25.0
            formula_valid = _formula_is_valid(formula, rule_type)
            if not formula_valid:
                issues.append("公式为空或格式不可解析")
                structure_score -= 20.0
            source_match_specs = _resolve_rule_field_refs(rule.get("source_fields") or [], source_alias_index)
            if rule_type not in CONST_RULE_TYPES and not source_match_specs:
                issues.append("源字段未在原协议 XML 中命中")
                structure_score -= 20.0
            if rule_type not in ALLOWED_RULE_TYPES:
                issues.append("规则类型不受支持")
                structure_score -= 15.0
            structure_score = max(structure_score, 0.0)
            structure_pass = structure_score >= 80.0

            declared_keys = {
                _normalize_lookup_value(item.field_name)
                for item in source_match_specs
            }
            resolved_source_count = len(declared_keys)
            all_sources_resolved = bool(declared_keys) and resolved_source_count == len(source_match_specs)
            matched_coarse = [
                item for item in top_candidates
                if _normalize_lookup_value(str(item.get("field_name") or "")) in declared_keys
            ]
            matched_fine = [
                item for item in accepted_candidates
                if _normalize_lookup_value(str(item.get("field_name") or "")) in declared_keys
            ]
            if rule_type in CONST_RULE_TYPES:
                field_match_score = 0.0
                semantic_score = 0.0
                dimension_score = 0.0
            else:
                dimension_score, dimension_pass = _score_dimension_consistency(target_field, source_match_specs, rule_type)
                if rule_type in EXPR_RULE_TYPES:
                    field_match_pass = all_sources_resolved and formula_valid
                    field_match_score = (
                        100.0 if field_match_pass else _safe_percent(resolved_source_count, max(len(declared_keys), 1))
                    )
                    if matched_fine:
                        semantic_score = _average_candidate_similarity(matched_fine, "fine_similarity")
                    elif matched_coarse:
                        semantic_score = max(_average_candidate_similarity(matched_coarse, "coarse_similarity"), 80.0)
                    elif field_match_pass:
                        semantic_score = 80.0
                    if not dimension_pass:
                        issues.append("量纲/物理语义一致性不足")
                    if field_match_pass and dimension_pass and structure_pass:
                        status = "pass"
                    else:
                        status = "fail"
                        if not field_match_pass:
                            issues.append("源字段解析或公式校验未通过")
                else:
                    semantic_score = _average_candidate_similarity(matched_fine, "fine_similarity")
                    field_match_pass = bool(declared_keys) and len(matched_fine) == len(declared_keys) and formula_valid
                    field_match_score = 100.0 if field_match_pass else (_safe_percent(len(matched_fine), max(len(declared_keys), 1)))
                    if not matched_fine:
                        issues.append("精排序未支持当前规则声明的源字段")
                    if not dimension_pass:
                        issues.append("量纲/物理语义一致性不足")
                    if field_match_pass and semantic_score >= fine_similarity_threshold * 100.0 and dimension_pass and structure_pass:
                        status = "pass"
                    else:
                        status = "fail"
        else:
            issues.append("目标字段缺少规则")
            field_match_score = 0.0
            semantic_score = 0.0
            dimension_score = 0.0
            structure_score = 0.0

        if rule and status == "fallback_zero":
            issues.append("无法可靠转换，规则显式置 0")
        declared_source_fields = list(rule.get("source_fields") or []) if rule else []
        is_convertible_rule = bool(rule) and bool(declared_source_fields)
        return {
            "field_result": {
                "target_field": target_field.field_name,
                "target_protocol": target_field.protocol_name,
                "rule_type": rule_type,
                "formula": formula,
                "status": status,
                "candidate_count": len(top_candidates),
                "coarse_hit": coarse_hit,
                "coarse_candidates": [
                    {
                        "field_name": item["field_name"],
                        "protocol_name": item["protocol_name"],
                        "coarse_similarity": round(float(item["coarse_similarity"]) * 100.0, 4),
                    }
                    for item in top_candidates[:5]
                ],
                "fine_candidates": [
                    {
                        "field_name": item["field_name"],
                        "protocol_name": item["protocol_name"],
                        "fine_similarity": round(float(item["fine_similarity"]) * 100.0, 4),
                    }
                    for item in accepted_candidates[:5]
                ],
                "declared_source_fields": list(rule.get("source_fields") or []) if rule else [],
                "resolved_source_fields": [item.field_name for item in source_match_specs],
                "field_match_correctness": _clamp_percent(field_match_score),
                "semantic_fidelity": _clamp_percent(semantic_score),
                "structure_integrity": _clamp_percent(structure_score),
                "dimension_consistency_accuracy": _clamp_percent(dimension_score),
                "issues": issues,
            },
            "coarse_hit_count": 1 if coarse_hit else 0,
            "fine_accept_count": 1 if rule and status == "pass" else 0,
            "covered_field_count": 1 if rule else 0,
            "converted_field_count": 1 if rule and status == "pass" else 0,
            "non_zero_rule_count": 1 if rule and not (rule_type in CONST_RULE_TYPES and formula == "0") else 0,
            "zero_fallback_count": 1 if rule_type in CONST_RULE_TYPES and formula == "0" else 0,
            "invalid_rule_count": 1 if rule and status == "fail" and not formula_valid else 0,
            "field_match_score": field_match_score,
            "semantic_score": semantic_score,
            "dimension_score": dimension_score,
            "structure_score": structure_score,
            "is_convertible_rule": is_convertible_rule,
        }

    executor: Optional[ThreadPoolExecutor] = None
    if normalized_max_workers > 1:
        executor = ThreadPoolExecutor(max_workers=normalized_max_workers)

    try:
        for batch_index in range(total_batches):
            start = batch_index * normalized_batch_size
            batch = target_fields[start : start + normalized_batch_size]
            batch_results: List[Dict[str, Any]] = []
            if executor is None:
                for target_field in batch:
                    batch_results.append(evaluate_target_field(target_field))
            else:
                future_map = {
                    executor.submit(evaluate_target_field, target_field): position
                    for position, target_field in enumerate(batch)
                }
                ordered_results: Dict[int, Dict[str, Any]] = {}
                for future in as_completed(future_map):
                    ordered_results[future_map[future]] = future.result()
                batch_results = [ordered_results[index] for index in sorted(ordered_results)]

            for evaluated in batch_results:
                coarse_recall_hit_count += evaluated["coarse_hit_count"]
                fine_accept_count += evaluated["fine_accept_count"]
                covered_field_count += evaluated["covered_field_count"]
                converted_field_count += evaluated["converted_field_count"]
                non_zero_rule_count += evaluated["non_zero_rule_count"]
                zero_fallback_count += evaluated["zero_fallback_count"]
                invalid_rule_count += evaluated["invalid_rule_count"]
                field_match_values.append(evaluated["field_match_score"])
                semantic_values.append(evaluated["semantic_score"])
                dimension_values.append(evaluated["dimension_score"])
                structure_values.append(evaluated["structure_score"])
                if evaluated["is_convertible_rule"]:
                    convertible_field_match_values.append(evaluated["field_match_score"])
                    convertible_semantic_values.append(evaluated["semantic_score"])
                    convertible_dimension_values.append(evaluated["dimension_score"])
                    convertible_structure_values.append(evaluated["structure_score"])
                if evaluated["converted_field_count"]:
                    successful_field_match_values.append(evaluated["field_match_score"])
                    successful_semantic_values.append(evaluated["semantic_score"])
                    successful_dimension_values.append(evaluated["dimension_score"])
                    successful_structure_values.append(evaluated["structure_score"])
                field_results.append(evaluated["field_result"])

            processed_targets += len(batch)
            _emit_progress(
                progress_callback,
                {
                    "stage": "evaluating",
                    "message": "规则评估进行中",
                    "completed_batches": batch_index + 1,
                    "total_batches": total_batches,
                    "processed_targets": processed_targets,
                    "target_field_count": len(target_fields),
                },
            )
    finally:
        if executor is not None:
            executor.shutdown(wait=True)

    existing_target_keys = {field.normalized_name for field in target_fields}
    for rule in rules:
        target_keys = rule.get("_target_match_keys") or []
        if any(target_key in existing_target_keys for target_key in target_keys):
            continue
        field_match_values.append(0.0)
        semantic_values.append(0.0)
        dimension_values.append(0.0)
        structure_values.append(0.0)
        status = "invalid_target"
        issues = ["规则目标字段未在目标协议 XML 中找到"]
        if _is_zero_fallback_rule(rule):
            status = "fallback_zero"
            zero_fallback_count += 1
            issues = ["无法可靠转换，规则显式置 0"]
        else:
            invalid_rule_count += 1
        field_results.append(
            {
                "target_field": rule["target_field"],
                "target_protocol": None,
                "rule_type": rule["rule_type"],
                "formula": rule["formula"],
                "status": status,
                "candidate_count": 0,
                "coarse_hit": False,
                "coarse_candidates": [],
                "fine_candidates": [],
                "declared_source_fields": list(rule.get("source_fields") or []),
                "resolved_source_fields": [],
                "field_match_correctness": 0.0,
                "semantic_fidelity": 0.0,
                "structure_integrity": 0.0,
                "dimension_consistency_accuracy": 0.0,
                "issues": issues,
            }
        )

    target_field_count = len(target_fields)
    convertible_field_count = len(convertible_field_match_values)
    successful_converted_field_count = len(successful_field_match_values)
    field_match_accuracy = _safe_percent(successful_converted_field_count, convertible_field_count)
    semantic_fidelity = _clamp_percent(sum(convertible_semantic_values) / max(convertible_field_count, 1))
    structure_integrity = _clamp_percent(sum(structure_values) / max(target_field_count, 1))
    dimension_accuracy = _clamp_percent(sum(convertible_dimension_values) / max(convertible_field_count, 1))
    field_coverage_rate = _safe_percent(covered_field_count, target_field_count)
    final_conversion_rate = _safe_percent(successful_converted_field_count, target_field_count)
    overall_correctness_score = _clamp_percent(
        field_match_accuracy * 0.25
        + semantic_fidelity * 0.20
        + structure_integrity * 0.20
        + dimension_accuracy * 0.15
        + final_conversion_rate * 0.20,
    )
    _emit_progress(
        progress_callback,
        {
            "stage": "completed",
            "message": "规则评估已完成",
            "processed_targets": processed_targets,
            "target_field_count": target_field_count,
            "convertible_field_count": convertible_field_count,
            "successful_converted_field_count": successful_converted_field_count,
            "final_conversion_rate": final_conversion_rate,
        },
    )

    return {
        "trace_id": trace,
        "export_path": export_path,
        "coarse_recall": {
            "candidate_top_k": int(coarse_top_k),
            "similarity_threshold": float(coarse_similarity_threshold),
            "recall_rate": _safe_percent(coarse_recall_hit_count, target_field_count),
            "hit_target_count": coarse_recall_hit_count,
            "target_field_count": target_field_count,
        },
        "fine_rerank": {
            "similarity_threshold": float(fine_similarity_threshold),
            "accepted_rule_count": fine_accept_count,
            "evaluated_rule_count": non_zero_rule_count,
        },
        "scores": {
            "field_match_accuracy": field_match_accuracy,
            "semantic_fidelity": semantic_fidelity,
            "structure_integrity": structure_integrity,
            "overall_correctness_score": overall_correctness_score,
            "dimension_consistency_accuracy": dimension_accuracy,
            "field_coverage_rate": field_coverage_rate,
            "final_conversion_rate": final_conversion_rate,
        },
        "summary": {
            "target_field_count": target_field_count,
            "rule_count": len(rules),
            "covered_field_count": covered_field_count,
            "converted_field_count": converted_field_count,
            "convertible_field_count": convertible_field_count,
            "successful_converted_field_count": successful_converted_field_count,
            "non_zero_rule_count": non_zero_rule_count,
            "zero_fallback_count": zero_fallback_count,
            "duplicate_target_count": len(duplicate_targets),
            "invalid_rule_count": invalid_rule_count,
            "embedding_parameter_count": embedding_model_meta.get("parameter_count"),
            "embedding_parameter_count_display": embedding_model_meta.get("parameter_count_display"),
            "reranker_parameter_count": reranker_model_meta.get("parameter_count"),
            "reranker_parameter_count_display": reranker_model_meta.get("parameter_count_display"),
        },
        "field_results": field_results,
        "strategy": {
            "model_inference_enabled": use_model_inference,
            "allow_modelscope_download": allow_modelscope_download,
            "batch_size": normalized_batch_size,
            "max_workers": normalized_max_workers,
            "embedding_backend": backend.embedding_info.__dict__,
            "reranker_backend": backend.reranker_info.__dict__,
            "embedding_model_meta": embedding_model_meta,
            "reranker_model_meta": reranker_model_meta,
            "degraded": not (backend.embedding_info.available and backend.reranker_info.available),
        },
    }
