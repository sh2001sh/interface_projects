...
def normalize_source_message(source_message: Any) -> Dict[str, Any]:
    """Normalize dict/list source messages into uppercase field-name keys."""
...
def _candidate_paths(model_kind: str) -> List[Path]:
    ...
    if model_kind == "embed":
        configured_model_dir = str(getattr(config, "EMBED_MODEL_DIR", "") or "").strip()
        if configured_model_dir:
            paths.append(Path(os.path.expanduser(configured_model_dir)))
        paths.extend(
            [
                configured_root / "Qwen" / "Qwen3-Embedding-0___6B",
                configured_root / "Qwen" / "Qwen3-Embedding-0.6B",
                configured_root / "Qwen3-Embedding-0___6B",
                configured_root / "Qwen3-Embedding-0.6B",
                home / "model_cache" / "Qwen" / "Qwen3-Embedding-0___6B",
                home / "model_cache" / "Qwen" / "Qwen3-Embedding-0.6B",
            ]
        )
    else:
        configured_model_dir = str(getattr(config, "RERANK_MODEL_DIR", "") or "").strip()
        if configured_model_dir:
            paths.append(Path(os.path.expanduser(configured_model_dir)))
        paths.extend(
            [
                configured_root / "Qwen" / "Qwen3-Reranker-0___6B",
                configured_root / "Qwen" / "Qwen3-Reranker-0.6B",
                configured_root / "Qwen3-Reranker-0___6B",
                configured_root / "Qwen3-Reranker-0.6B",
                home / "model_cache" / "Qwen3-Reranker-0___6B",
...
def _load_backends(use_model_inference: bool, allow_modelscope_download: bool) -> EvaluationBackend:
    ...
    if not embedding_info.available or not reranker_info.available:
        reasons = []
        if not embedding_info.available:
            reasons.append(f"embedding: {embedding_info.reason or '未知错误'}")
        if not reranker_info.available:
            reasons.append(f"reranker: {reranker_info.reason or '未知错误'}")
        raise RuntimeError("评估模型加载失败: " + "; ".join(reasons))
...
def evaluate_protocol_conversion(
    ...
    converted_denominator = max(converted_weight, 1.0)
    ...
    correctness_score = weighted_correctness / converted_denominator * 100.0
    semantic_score = weighted_semantic / converted_denominator * 100.0
    rerank_score = weighted_rerank / converted_denominator * 100.0
