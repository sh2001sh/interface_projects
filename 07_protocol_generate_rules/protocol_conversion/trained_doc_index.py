...
class TrainedDocEvidenceProvider(PageIndexEvidenceProvider):
    ...
    def __init__(
        ...
        self.doc_set_id = str(doc_set_id or "").strip()
        self.index_ref = str(index_ref or "").strip()
        ...
        self._has_explicit_registry_path = has_index_registry_path
        ...
        if has_index_registry_path:
            ...
            if resolved_registry:
                normalized_registry = dict(resolved_registry)
                _normalize_registry_storage_paths(normalized_registry)
                registries = [normalized_registry]
            else:
                registries = []
    ...
    def collect_evidence(
        ...
        self._ensure_registry_for_context(
        ...
        if not snippets and not self._has_explicit_registry_path:
            previous_registry_paths = tuple(self.registry.get("registry_paths") or [])
            self._ensure_registry_for_context(
                source_protocol=source_protocol,
                target_protocol=target_protocol,
                source_message=source_message,
            ...
            if tuple(self.registry.get("registry_paths") or []) != previous_registry_paths:
                snippets = self._collect_source_registry_snippets(
                    source_protocol=source_protocol,
                    queries=source_queries,
                    top_k=max_snippets_per_role,
    ...
    def _set_active_registry(self, registries: List[Dict[str, Any]]) -> None:
        self.registry = _merge_pageindex_registries(registries)
        workspace_dir = self.registry.get("workspace_dir") if isinstance(self.registry, dict) else None
        docs_dir = self.registry.get("docs_dir") if isinstance(self.registry, dict) else None
    ...
    def _ensure_registry_for_context(
        self,
        source_protocol: Dict[str, Any],
        target_protocol: Dict[str, Any],
        source_message: Optional[Any] = None,
        ...
        if self._has_explicit_registry_path:
            return
        if self.registry and not force:
            return
        registries = self._discover_relevant_registries(
            source_protocol=source_protocol,
            target_protocol=target_protocol,
            source_message=source_message,
        )
        if registries:
            self._set_active_registry(registries)

    def _discover_relevant_registries(
        self,
        source_protocol: Dict[str, Any],
        target_protocol: Dict[str, Any],
        source_message: Optional[Any] = None,
        max_registry_count: int = 3,
    ) -> List[Dict[str, Any]]:
        self._discovery_attempt_count += 1
        candidate_registries = self._list_candidate_registries()
        if not candidate_registries:
            return []

        query_terms = self._build_discovery_queries(
            source_protocol=source_protocol,
            target_protocol=target_protocol,
            source_message=source_message,
        )
        ...
        for registry in candidate_registries:
            registry_item = dict(registry)
            _normalize_registry_storage_paths(registry_item)
            score = self._score_registry_candidate(
            ...
            if score <= 0:
                continue
            registry_item["_auto_discovery_score"] = score
            scored.append(registry_item)
        ...
        scored.sort(
            key=lambda item: (
                -float(item.get("_auto_discovery_score") or 0.0),
                -int(item.get("document_count") or 0),
                str(item.get("doc_set_id") or ""),
            )
        )
        return scored[: max(1, max_registry_count)]

    def _list_candidate_registries(self) -> List[Dict[str, Any]]:
        prioritized = self.file_store.list_all_pageindex_registries(
            project_id=self.project_id,
            dataset_id=self.dataset_id,
            doc_set_id=self.doc_set_id,
            index_ref=self.index_ref,
        ...
        if self.project_id:
            project_only = self.file_store.list_all_pageindex_registries(project_id=self.project_id)
            if project_only:
                return project_only

        return self.file_store.list_all_pageindex_registries()
    ...
    def _build_discovery_queries(
        ...
        def push(value: Any) -> None:
            normalized = _compact_text(value).upper()
            if not normalized or normalized in seen:
                return
            seen.add(normalized)
            queries.append(normalized)

        for item in self._extract_source_queries(source_protocol, source_message):
            push(item)
        for item in self._extract_target_queries(target_protocol):
            push(item)
        for value in (
            source_protocol.get("protocol_type"),
            source_protocol.get("name"),
            source_protocol.get("message_code"),
            target_protocol.get("protocol_type"),
            target_protocol.get("name"),
            target_protocol.get("message_code"),
        ):
            push(value)
        return queries
    ...
    def _score_registry_candidate(
        ...
        registry_dataset_id = str(registry.get("dataset_id") or "").strip()
        registry_doc_set_id = str(registry.get("doc_set_id") or "").strip()
        registry_index_ref = str(registry.get("index_ref") or "").strip()
        registry_protocol_type = str(registry.get("protocol_type") or "").strip()
        source_protocol_type = str(source_protocol.get("protocol_type") or "").strip()
        source_message_code = str(source_protocol.get("message_code") or "").strip()

        if self.project_id and registry_project_id == self.project_id:
            score += 6.0
        if self.dataset_id and registry_dataset_id == self.dataset_id:
            score += 8.0
        if self.doc_set_id and registry_doc_set_id == self.doc_set_id:
            score += 10.0
        if self.index_ref and registry_index_ref == self.index_ref:
            score += 8.0
        if source_protocol_type and registry_protocol_type == source_protocol_type:
            score += 5.0
        if source_message_code and source_message_code in _normalize_message_codes(registry.get("message_codes")):
            score += 4.0

        candidate_documents = self._filter_registry_documents(source_protocol, registry=registry)
        if not candidate_documents:
            return round(score, 4)

        ranked_documents = self._rank_registry_documents(candidate_documents, queries)
        top_scores: List[float] = []
        for document in ranked_documents[:DEFAULT_SCAN_SHARD_LIMIT]:
            shard_score = float(document.get("_registry_score") or _score_document_shard(document, queries))
            if shard_score <= 0:
                continue
            top_scores.append(shard_score)
        if top_scores:
            score += sum(top_scores[:4])
            score += min(len(top_scores), 4) * 0.25
        elif queries:
            return 0.0
        else:
            score += min(len(candidate_documents), 4) * 0.25
        return round(score, 4)
    ...
    def _filter_registry_documents(
        self,
        source_protocol: Dict[str, Any],
        registry: Optional[Dict[str, Any]] = None,
    ) -> List[Dict[str, Any]]:
        active_registry = registry if isinstance(registry, dict) else self.registry
        documents = list((active_registry.get("documents") or [])) if isinstance(active_registry, dict) else []
