...
from .converter import (
    ...
    _strip_balanced_outer_parentheses,
...
def _strip_explicit_target_assignment(
    ...
    aliases = _target_field_assignment_aliases(target_field, target_protocol_type)
    target_leaf = _strip_protocol_like_field_prefix(target_field)
    protocol_prefixes = {
        _normalize_rule_field_token(item)
        for item in (
            str(target_protocol_type or "").strip(),
            str(target_protocol_type or "").strip().replace(".", "_"),
            str(target_protocol_type or "").strip().replace(".", ""),
            _normalize_protocol_prefix(target_protocol_type),
            _normalize_protocol_prefix(target_protocol_type).replace("_", ""),
        )
        if str(item or "").strip()
    }
    ...
    if not right or (left not in aliases and not left_matches_loop_target):
        return text
    return right


def _target_identifier_matches(
    token: Any,
    target_field: Any,
    target_protocol_type: Optional[str],
) -> bool:
    normalized = _normalize_rule_field_token(token)
    if not normalized:
        return False
    if normalized in _target_field_assignment_aliases(target_field, target_protocol_type):
        return True
    target_leaf = _strip_protocol_like_field_prefix(target_field)
    protocol_prefixes = {
        _normalize_rule_field_token(item)
    ...
    return bool(
        target_leaf
        and normalized.endswith(f"_{target_leaf}")
        and (
            any(normalized.startswith(f"{prefix}_") or normalized.startswith(f"{prefix}.") for prefix in protocol_prefixes)
            or re.search(r"(?:^|_)循环\d+_\d+_", normalized, flags=re.IGNORECASE)
        )
    )
...
def _normalize_python_block_target_assignments(
    ...
    for raw_line in str(formula or "").splitlines():
        ...
        if match and _target_identifier_matches(match.group(2), target_field, target_protocol_type):
            lines.append(f"{match.group(1)}result = {match.group(3).strip()}")
            changed = True
        else:
            lines.append(raw_line)
    return "\n".join(lines) if changed else str(formula or "")


def _target_field_reference_aliases(
    target_field: Any,
    target_protocol_type: Optional[str] = None,
) -> set[str]:
    normalized_target = _normalize_rule_field_token(target_field)
    ...
    aliases = set(_target_field_assignment_aliases(normalized_target, target_protocol_type))
    stripped_target = _strip_protocol_like_field_prefix(normalized_target)
    if stripped_target:
        aliases.add(stripped_target)
    ...
    protocol_candidates = {
        ...
        _normalize_protocol_prefix(raw_protocol),
        _normalize_protocol_prefix(raw_protocol).replace("_", ""),
    }
    for prefix in {item for item in protocol_candidates if item}:
        for separator in ("_", "."):
            aliases.add(_normalize_rule_field_token(f"{prefix}{separator}{normalized_target}"))
            if stripped_target:
                aliases.add(_normalize_rule_field_token(f"{prefix}{separator}{stripped_target}"))

    return {item for item in aliases if item}


def _formula_tokens(formula: Any) -> List[str]:
    return re.findall(r"[A-Za-z_\u4E00-\u9FFF][A-Za-z0-9_\u4E00-\u9FFF.]*", str(formula or ""))


def _formula_references_target_field(
    formula: Any,
    target_field: Any,
    target_protocol_type: Optional[str],
    source_fields: Iterable[Any],
) -> bool:
    target_aliases = _target_field_reference_aliases(target_field, target_protocol_type)
    if not target_aliases:
        return False
    source_aliases: set[str] = set()
    for source_field in source_fields:
        source_aliases.update(_rule_field_lookup_keys(source_field))
    target_leaf = _strip_protocol_like_field_prefix(target_field)
    protocol_prefixes = {
        _normalize_rule_field_token(item)
        for item in (
    ...
    for token in _formula_tokens(formula):
        normalized = _normalize_rule_field_token(token)
        if normalized in target_aliases and normalized not in source_aliases:
            return True
        if _target_identifier_matches(normalized, target_field, target_protocol_type) and normalized not in source_aliases:
            return True
    return False


def _is_zero_literal(value: Any) -> bool:
    return bool(re.fullmatch(r"0(?:\.0|U|L)?", str(value or "").strip(), flags=re.IGNORECASE))


def _strip_self_referential_target_guard_at_source(
    formula: str,
    target_field: Any,
    target_protocol_type: Optional[str],
    source_fields: Iterable[Any],
) -> str:
    text = _strip_balanced_outer_parentheses(formula)
    ternary_parts = _split_top_level_ternary(text)
    if ternary_parts is None:
        ...
        if not isinstance(parsed.body, ast.IfExp):
            return formula
        ternary_parts = (
            ast.unparse(parsed.body.test),
            ast.unparse(parsed.body.body),
            ast.unparse(parsed.body.orelse),
        )
    condition, when_true, when_false = ternary_parts
    if not _is_zero_literal(_strip_balanced_outer_parentheses(when_false)):
        return formula
    if _formula_references_target_field(when_true, target_field, target_protocol_type, source_fields):
        return formula
    equality = re.fullmatch(r"(.+?)(?:==|!=)(.+)", _strip_balanced_outer_parentheses(condition))
    if not equality:
        return formula
    left = _strip_balanced_outer_parentheses(equality.group(1))
    right = _strip_balanced_outer_parentheses(equality.group(2))
    left_is_target = _formula_references_target_field(left, target_field, target_protocol_type, source_fields)
    right_is_target = _formula_references_target_field(right, target_field, target_protocol_type, source_fields)
    if left_is_target == right_is_target:
        return formula
    return _strip_balanced_outer_parentheses(when_true)
...
def normalize_generated_rules(rule_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in rule_items:
        ...
        if formula_kind == "python_expr":
            ...
            rule = _strip_self_referential_target_guard_at_source(
                rule,
                target_field=target_field,
                target_protocol_type=str(item.get("target_protocol_type") or "").strip() or None,
            ...
            rule = _normalize_python_block_target_assignments(
                rule,
                target_field=target_field,
                target_protocol_type=str(item.get("target_protocol_type") or "").strip() or None,
            )
...
def _knowledge_rule_to_generated_rule(rule: Any) -> Dict[str, Any]:
    ...
    if formula_kind == "python_expr":
        ...
        normalized_formula = _strip_self_referential_target_guard_at_source(
            normalized_formula,
            target_field=target_field,
            target_protocol_type=str(getattr(rule, "target_protocol_type", "") or "").strip() or None,
            source_fields=source_fields,
        ...
        normalized_formula = _normalize_python_block_target_assignments(
            normalized_formula,
            target_field=target_field,
            target_protocol_type=str(getattr(rule, "target_protocol_type", "") or "").strip() or None,
        )
    ...
    for rule in generated_rules:
        ...
        if explicit_target_actual or explicit_target_path:
            ...
            if matched_instance is None:
                explicit_leaf = (
                    explicit_target_path.split("/")[-1].strip()
                    if explicit_target_path
                    else str(rule.get("target_field") or "").strip()
                )
                fallback_matches = [
                    instance
                    for instance in target_instances
                    if explicit_leaf
                    and explicit_leaf in {
                        str(instance.get("field_name") or "").strip(),
                        str(instance.get("label") or "").strip(),
                        str(((instance.get("path_parts") or [None])[-1]) or "").strip(),
                        str(instance.get("target_path") or "").strip().split("/")[-1].strip(),
                    }
                ]
                if len(fallback_matches) == 1:
                    matched_instance = fallback_matches[0]
                elif len(target_instances) == 1:
                    matched_instance = target_instances[0]
            if matched_instance is None:
                continue
...
def _validate_rule_formula_fields(
    ...
    if formula_kind == "python_expr":
        formula = _normalize_formula_expression_syntax(formula)
        formula = _strip_self_referential_target_guard_at_source(
            formula,
        ...
        formula = _normalize_python_block_target_assignments(
            formula,
            target_field=rule.get("target_field"),
            target_protocol_type=str(rule.get("target_protocol_type") or "").strip() or None,
        )
        rule["rule"] = formula
    if _formula_references_target_field(
        formula,
        target_field=rule.get("target_field"),
        target_protocol_type=str(rule.get("target_protocol_type") or "").strip() or None,
        source_fields=source_fields,
    ):
        return False, "公式表达式不能引用目标字段，目标字段只能作为赋值左值"
...
def _filter_valid_generated_rules(
    ...
    for rule in generated_rules:
        ...
        if not is_valid and "目标字段" not in str(reason or ""):
...
def generate_protocol_field_rules(
    ...
    semantic_match_time_ms_total = 0.0
    rule_generation_time_ms_total = 0.0
    rule_generation_count_total = 0
    graph_rules = []
    ...
    if use_knowledge_base and catalog_protocol_groups:
        ...
        for protocol_name, message_code, lookup_fields in catalog_protocol_groups:
            ...
            try:
                ...
                lookup_result = protocol_kb.lookup_rules_for_source_fields(
                    source_fields=normalized_lookup_fields,
                    message_code=message_code,
                    target_protocol_type=target_protocol_type,
                    target_message_code=normalized_target_protocol.get("message_code"),
                    target_fields=normalized_required_target_names or None,
                )
                graph_rules.extend(lookup_result.rules)
                semantic_match_query_count += int(lookup_result.metrics.semantic_query_count or 0)
                semantic_match_time_ms_total += float(lookup_result.metrics.semantic_match_time_ms or 0.0)
                rule_generation_count_total += int(lookup_result.metrics.rule_generation_count or 0)
                rule_generation_time_ms_total += float(lookup_result.metrics.rule_generation_time_ms or 0.0)
            except Exception as exc:
        ...
        lookup_result = knowledge_base.lookup_rules_for_source_fields(
        ...
        graph_rules = lookup_result.rules
        semantic_match_query_count = int(lookup_result.metrics.semantic_query_count or 0)
        semantic_match_time_ms_total = float(lookup_result.metrics.semantic_match_time_ms or 0.0)
        rule_generation_count_total = int(lookup_result.metrics.rule_generation_count or 0)
        rule_generation_time_ms_total = float(lookup_result.metrics.rule_generation_time_ms or 0.0)
    ...
    semantic_match_avg_query_time_ms = round(
        semantic_match_time_ms_total / semantic_match_query_count,
        4,
    ) if semantic_match_query_count > 0 else None
    if graph_rules:
        ...
        if graph_generated_rules and not _missing_target_concepts(graph_generated_rules, concept_required_target_fields):
            ...
            return {
                ...
                "summary": {
                    ...
                    "semantic_match_query_count": semantic_match_query_count,
                    "semantic_match_time_ms_total": round(semantic_match_time_ms_total, 4),
                    "semantic_match_avg_query_time_ms": semantic_match_avg_query_time_ms,
                    "semantic_match_time_target_met": (
                        semantic_match_avg_query_time_ms <= 20.0
                        if semantic_match_avg_query_time_ms is not None
                        else None
                    ),
                    "knowledge_graph_lookup_time_ms": graph_lookup_time_ms,
                    "rule_generation_target_count": rule_generation_count_total,
                    "rule_generation_time_ms_total": round(rule_generation_time_ms_total, 4),
                    "rule_generation_avg_time_ms": round(
                        rule_generation_time_ms_total / rule_generation_count_total,
                        4,
                    ) if rule_generation_count_total > 0 else None,
                    "rule_generation_time_target_met": (
                        (round(rule_generation_time_ms_total / rule_generation_count_total, 4) <= 50.0)
                        if rule_generation_count_total > 0
                        else None
                    ),
                    ...
                    "knowledge_graph_rule_time_target_met": (
                        (round(rule_generation_time_ms_total / rule_generation_count_total, 4) <= 50.0)
                        if rule_generation_count_total > 0
                        else None
                    ),
    ...
    rule_generation_target_count = rule_generation_count_total
    rule_generation_avg_time_ms = round(
        rule_generation_time_ms_total / rule_generation_target_count,
        4,
    ) if rule_generation_target_count > 0 else None
    return {
        "source_protocol": normalized_source_protocol,
        "target_protocol": normalized_target_protocol,
        ...
        "summary": {
            ...
            "knowledge_graph_lookup_time_ms": graph_lookup_time_ms,
            "rule_generation_target_count": rule_generation_target_count,
            "rule_generation_time_ms_total": round(rule_generation_time_ms_total, 4),
            "rule_generation_avg_time_ms": rule_generation_avg_time_ms,
            "rule_generation_time_target_met": (
                rule_generation_avg_time_ms <= 50.0
                if rule_generation_avg_time_ms is not None
                else None
            ),
