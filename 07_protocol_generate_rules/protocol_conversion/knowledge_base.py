...
@dataclass
class KnowledgeRuleLookupMetrics:
    semantic_query_count: int = 0
    semantic_match_time_ms: float = 0.0
    rule_generation_count: int = 0
    rule_generation_time_ms: float = 0.0


@dataclass
class KnowledgeRuleLookupResult:
    rules: List[KnowledgeRule] = field(default_factory=list)
    metrics: KnowledgeRuleLookupMetrics = field(default_factory=KnowledgeRuleLookupMetrics)
...
class ProtocolConversionKnowledgeBase:
    ...
    @classmethod
    def _target_formula_aliases(cls, target_field: Any, target_protocol_type: Optional[str] = None) -> set[str]:
        normalized = cls._normalize_field_name(target_field)
        if not normalized:
            return set()
        aliases = set(cls._target_field_keys(normalized, target_protocol_type))
        stripped = cls._normalize_field_name(cls._strip_protocol_field_prefix(normalized, target_protocol_type))
        if stripped:
            aliases.add(stripped)

        raw_protocol = str(target_protocol_type or "").strip()
        normalized_protocol = cls._normalize_protocol_prefix(raw_protocol)
        protocol_candidates = {
            raw_protocol,
        ...
        for prefix in {item for item in protocol_candidates if item}:
            for separator in ("_", "."):
                aliases.add(cls._normalize_field_name(f"{prefix}{separator}{normalized}"))
                if stripped:
                    aliases.add(cls._normalize_field_name(f"{prefix}{separator}{stripped}"))
    ...
    def _strip_explicit_target_assignment(
        ...
        if not match:
            return text
        left = cls._normalize_field_name(match.group(1))
        target_leaf = cls._normalize_field_name(cls._strip_protocol_field_prefix(target_field, target_protocol_type))
        raw_protocol = str(target_protocol_type or "").strip()
        normalized_protocol = cls._normalize_protocol_prefix(raw_protocol)
        protocol_prefixes = {
        ...
        left_matches_loop_target = bool(
            target_leaf
            and left.endswith(f"_{target_leaf}")
            and (
                any(left.startswith(f"{prefix}_") or left.startswith(f"{prefix}.") for prefix in protocol_prefixes)
                or re.search(r"(?:^|_)循环\d+_\d+_", left, flags=re.IGNORECASE)
            )
        )
        if left not in cls._target_formula_aliases(target_field, target_protocol_type) and not left_matches_loop_target:
            return text
        return str(match.group(2) or "").strip()

    @classmethod
    def _normalize_python_block_target_assignments(
        cls,
        formula: Any,
        target_field: Any,
        target_protocol_type: Optional[str] = None,
        ...
        for raw_line in str(formula or "").splitlines():
            ...
            if match and match.group(2).upper() in cls._target_formula_aliases(target_field, target_protocol_type):
                lines.append(f"{match.group(1)}result = {match.group(3).strip()}")
                changed = True
            else:
                lines.append(raw_line)
        return "\n".join(lines) if changed else str(formula or "").strip()
    ...
    def _build_rule_index(self) -> Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]]:
        index: Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]] = defaultdict(list)
        for edge in self._edges:
            rule = self._edge_to_rule(edge)
            if rule is None:
                continue
            protocol = self._normalize_protocol_type(rule.protocol_type or self.protocol_type)
            message_code = self._normalize_message_code(rule.message_code)
            source_keys: set[str] = set()
            for source_name in (rule.source_fields or [rule.field_name]):
                source_keys.update(self._source_field_keys(source_name, protocol))
            source_keys.update(self._source_field_keys(rule.field_name, protocol))
            for alias in rule.aliases:
                source_keys.update(self._source_field_keys(alias, protocol))
            for key in source_keys:
                index[(protocol, message_code, key)].append(rule)
                if message_code is not None:
                    index[(protocol, None, key)].append(rule)
        return {key: list(value) for key, value in index.items()}
    ...
    def _iter_rule_source_keys(self, rule: KnowledgeRule) -> set[str]:
        protocol = self._normalize_protocol_type(rule.protocol_type or self.protocol_type)
        keys: set[str] = set()
        for source_name in (rule.source_fields or [rule.field_name]):
            keys.update(self._source_field_keys(source_name, protocol))
        keys.update(self._source_field_keys(rule.field_name, protocol))
        for alias in rule.aliases:
            keys.update(self._source_field_keys(alias, protocol))
        return keys

    def _build_rule_lookup_index(self, rules: Iterable[KnowledgeRule]) -> Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]]:
        index: Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]] = defaultdict(list)
        for rule in rules:
            protocol = self._normalize_protocol_type(rule.protocol_type or self.protocol_type)
            message_code = self._normalize_message_code(rule.message_code)
            for key in self._iter_rule_source_keys(rule):
                index[(protocol, message_code, key)].append(rule)
                if message_code is not None:
                    index[(protocol, None, key)].append(rule)
        return {key: list(value) for key, value in index.items()}
    ...
    def _score_rule_candidate(
        self,
        rule: KnowledgeRule,
        matched_source_key: Optional[str],
        normalized_message: Optional[str],
        normalized_target_protocol: Optional[str],
        normalized_target_message: Optional[str],
        ...
        if normalized_message and rule.message_code == normalized_message:
            ...
            score += 8
        rule_source_keys = self._iter_rule_source_keys(rule)
        if matched_source_key and matched_source_key in rule_source_keys:
            score += 12
    ...
    def _lookup_rules_from_index(
        self,
        index: Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]],
        source_fields: Iterable[str],
        message_code: Optional[str] = None,
        target_protocol_type: Optional[str] = None,
        target_message_code: Optional[str] = None,
        target_fields: Optional[List[str]] = None,
    ) -> KnowledgeRuleLookupResult:
        wanted: set[str] = set()
        for item in source_fields:
            wanted.update(self._source_field_keys(item, self.protocol_type))
        if not wanted:
            return KnowledgeRuleLookupResult()
        ...
        indexed_candidates: Dict[Tuple[str, Tuple[str, ...], str], Tuple[Optional[str], KnowledgeRule]] = {}
        for source_key in wanted:
            lookup_keys = (
                [(self.protocol_type, normalized_message, source_key), (self.protocol_type, None, source_key)]
                if normalized_message is not None
                else [(self.protocol_type, None, source_key)]
            )
            for lookup_key in lookup_keys:
                for rule in index.get(lookup_key, []):
                    signature = self._rule_signature(rule)
                    if signature not in indexed_candidates:
                        indexed_candidates[signature] = (source_key, rule)
        semantic_time_ms = round((time.perf_counter() - semantic_started) * 1000.0, 4)

        rule_started = time.perf_counter()
        candidates: List[Tuple[int, KnowledgeRule]] = []
        seen = set()
        for matched_source_key, rule in indexed_candidates.values():
            ...
            candidates.append(
                (
                    self._score_rule_candidate(
                        ...
                        normalized_target_protocol=normalized_target_protocol,
                        normalized_target_message=normalized_target_message,
        ...
        for _score, rule in candidates:
            ...
            if any(not group or group.isdisjoint(wanted) for group in source_field_groups):
                continue
            signature = self._rule_signature(rule)
            if signature in matched_seen:
                continue
            matched_seen.add(signature)
            matched.append(rule)
        rule_time_ms = round((time.perf_counter() - rule_started) * 1000.0, 4)
        return KnowledgeRuleLookupResult(
            rules=matched,
            metrics=KnowledgeRuleLookupMetrics(
                semantic_query_count=len(wanted),
                semantic_match_time_ms=semantic_time_ms,
                rule_generation_count=len(matched),
                rule_generation_time_ms=rule_time_ms,
            ),
        )

    def lookup_rules_for_source_fields(
        self,
        source_fields: Iterable[str],
        message_code: Optional[str] = None,
        target_protocol_type: Optional[str] = None,
        ...
        return self._lookup_rules_from_index(
            getattr(self, "_rule_index", {}),
    ...
    def find_rules_for_source_fields(
        ...
        return self.lookup_rules_for_source_fields(
            source_fields=source_fields,
            message_code=message_code,
            target_protocol_type=target_protocol_type,
            target_message_code=target_message_code,
    ...
    def _normalize_rule_input(
        ...
        formula = self._strip_explicit_target_assignment(formula, target_field, target_protocol_value)
        if self._infer_formula_kind(formula) == "python_block":
            formula = self._normalize_python_block_target_assignments(formula, target_field, target_protocol_value)
...
class Neo4jProtocolConversionKnowledgeBase(ProtocolConversionKnowledgeBase):
    ...
    def __init__(self, protocol_type: str, settings: KnowledgeGraphSettings):
        ...
        self._external_rule_index: Optional[Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]]] = None
    ...
    def _invalidate_external_cache(self) -> None:
        ...
        self._external_rule_index = None

    def _collect_external_mapto_rules(
        self,
        records: List[Dict[str, Any]],
        ...
        for record in records:
            ...
            formula = self._strip_explicit_target_assignment(formula, target_field, target_protocol)
            if self._infer_formula_kind(formula) == "python_block":
                formula = self._normalize_python_block_target_assignments(formula, target_field, target_protocol)
            formula = self._strip_known_prefixed_formula_identifiers(
                formula,
                list(source_fields),
            )
    ...
    def _build_external_mapto_rules(self) -> List[KnowledgeRule]:
        ...
        return self._collect_external_mapto_rules(records, self._load_external_alias_map())

    def _collect_external_expression_rules(
        self,
        records: List[Dict[str, Any]],
        alias_map: Dict[str, List[str]],
    ) -> List[KnowledgeRule]:
    ...
    def _build_external_expression_rules(self) -> List[KnowledgeRule]:
        ...
        return self._collect_external_expression_rules(records, self._load_external_alias_map())
    ...
    def _load_external_rule_index(self) -> Dict[Tuple[str, Optional[str], str], List[KnowledgeRule]]:
        if self._external_rule_index is not None:
            return self._external_rule_index
        self._external_rule_index = self._build_rule_lookup_index(self._load_external_rules())
        return self._external_rule_index

    def _query_external_alias_map(self, source_names: set[str]) -> Dict[str, List[str]]:
        normalized_sources = {self._normalize_field_name(item) for item in source_names if self._normalize_field_name(item)}
        ...
        try:
            records = self._run_cypher(
                """
                MATCH (a:Entity)-[:D]-(b:Entity)
                WHERE toUpper(a.name) IN $source_names OR toUpper(b.name) IN $source_names
                RETURN a.name AS source_name, b.name AS target_name
                """,
        ...
        for record in records:
            ...
            alias_graph[source_name].add(target_name)
            alias_graph[target_name].add(source_name)
        return {key: sorted(values) for key, values in alias_graph.items()}

    def _query_external_source_names(self, source_names: set[str]) -> set[str]:
        normalized_sources = {self._normalize_field_name(item) for item in source_names if self._normalize_field_name(item)}
        if not normalized_sources:
            return set()
        if self.schema_mode == PROTOCOLFIELD_GRAPH_MODE:
            records = self._run_cypher(
                """
                MATCH (src:ProtocolField)
                WHERE toUpper(src.name) IN $source_names
                RETURN DISTINCT src.name AS source_name
                """,
                {"source_names": sorted(normalized_sources)},
            )
        else:
        ...
        return {
            self._normalize_field_name(self._strip_protocol_field_prefix(item.get("source_name")))
            for item in records
            if self._normalize_field_name(self._strip_protocol_field_prefix(item.get("source_name")))
        }

    def _query_external_mapto_rule_records(
        self,
        ...
        return self._run_cypher(
            ...
            MATCH (src:ProtocolField)-[r:MAP_TO]->(target:ProtocolField)
            WHERE toUpper(src.name) IN $source_names
              AND ($has_target_filter = false OR toUpper(target.name) IN $target_names)
            RETURN src.name AS source_name, target.name AS target_name, properties(r) AS rel_props
            """,
            {
                "source_names": sorted(source_names),
                "target_names": sorted(target_names),
                "has_target_filter": bool(target_names),
            },
        )

    def _query_external_expression_rule_records(
        self,
        source_names: set[str],
        target_names: set[str],
    ) -> List[Dict[str, Any]]:
    ...
    def _lookup_external_rules_via_neo4j(
        self,
        source_fields: Iterable[str],
        message_code: Optional[str] = None,
        target_protocol_type: Optional[str] = None,
        ...
        semantic_started = time.perf_counter()
        matched_sources = self._query_external_source_names(wanted)
        alias_map = self._query_external_alias_map(matched_sources or wanted)
        expanded_sources = set(matched_sources or wanted)
        for source_name in list(expanded_sources):
            expanded_sources.update(alias_map.get(source_name, []))
        semantic_time_ms = round((time.perf_counter() - semantic_started) * 1000.0, 4)

        rule_started = time.perf_counter()
        mapto_records: List[Dict[str, Any]] = []
        if self.schema_mode == PROTOCOLFIELD_GRAPH_MODE:
            mapto_records = self._query_external_mapto_rule_records(expanded_sources, normalized_targets)
        expression_records = self._query_external_expression_rule_records(expanded_sources, normalized_targets)
        rule_query_time_ms = round((time.perf_counter() - rule_started) * 1000.0, 4)

        rules: List[KnowledgeRule] = []
        if mapto_records:
            rules.extend(self._collect_external_mapto_rules(mapto_records, alias_map))
        expression_rules = self._collect_external_expression_rules(expression_records, alias_map)
        existing_signatures = {self._rule_signature(rule) for rule in rules}
        for rule in expression_rules:
            ...
            existing_signatures.add(signature)
            rules.append(rule)
        lookup_result = self._lookup_rules_from_index(
            self._build_rule_lookup_index(rules),
            source_fields=source_fields,
            message_code=message_code,
            target_protocol_type=target_protocol_type,
            target_message_code=target_message_code,
        ...
        return KnowledgeRuleLookupResult(
            rules=lookup_result.rules,
            metrics=KnowledgeRuleLookupMetrics(
                semantic_query_count=len(wanted),
                semantic_match_time_ms=semantic_time_ms,
                rule_generation_count=len(lookup_result.rules),
                rule_generation_time_ms=rule_query_time_ms,
            ),
        )
    ...
    def _record_to_rule(self, record: Dict[str, Any]) -> Optional[KnowledgeRule]:
        ...
        normalized_formula = self._strip_explicit_target_assignment(
            normalized_formula,
            normalized_target_field,
            target_protocol_value,
        )
        if self._infer_formula_kind(normalized_formula) == "python_block":
            normalized_formula = self._normalize_python_block_target_assignments(
                normalized_formula,
                normalized_target_field,
                target_protocol_value,
            )
    ...
    def _lookup_rules_from_records(
        self,
        source_fields: Iterable[str],
        message_code: Optional[str] = None,
        target_protocol_type: Optional[str] = None,
        ...
        normalized_target_protocol = str(target_protocol_type or "").strip() or None
        normalized_target_message = self._normalize_message_code(target_message_code)
        allowed_statuses = set(self.settings.read_statuses or [])

        semantic_started = time.perf_counter()
        records = self._query_rule_records()
        semantic_time_ms = round((time.perf_counter() - semantic_started) * 1000.0, 4)
        ...
        for record in records:
            ...
            for alias in rule.aliases:
                source_names.update(self._source_field_keys(alias, rule.protocol_type))
            matched_source_keys = source_names.intersection(wanted)
            if wanted and not matched_source_keys:
                continue
            if normalized_message and rule.message_code not in {None, normalized_message}:
                continue
            if normalized_target_protocol and rule.target_protocol_type not in {None, normalized_target_protocol}:
            ...
            candidates.append(
                (
                    self._score_rule_candidate(
                        rule,
                        matched_source_key=sorted(matched_source_keys)[0] if matched_source_keys else None,
                        normalized_message=normalized_message,
                        normalized_target_protocol=normalized_target_protocol,
                        normalized_target_message=normalized_target_message,
                        normalized_targets=normalized_targets,
        ...
        candidates.sort(key=lambda item: (-item[0], item[1].target_field or item[1].field_name, item[1].formula))
        matched = [rule for _, rule in candidates]
        rule_time_ms = round((time.perf_counter() - rule_started) * 1000.0, 4)
        return KnowledgeRuleLookupResult(
            rules=matched,
            metrics=KnowledgeRuleLookupMetrics(
    ...
    def lookup_rules_for_source_fields(
        ...
        if self._uses_external_entity_graph(self.schema_mode):
            return self._lookup_external_rules_via_neo4j(
                source_fields=source_fields,
                message_code=message_code,
                target_protocol_type=target_protocol_type,
                target_message_code=target_message_code,
        ...
        return self._lookup_rules_from_records(
            source_fields=source_fields,
            message_code=message_code,
            target_protocol_type=target_protocol_type,
            target_message_code=target_message_code,
...
class CompositeProtocolConversionKnowledgeBase(ProtocolConversionKnowledgeBase):
    ...
    def lookup_rules_for_source_fields(
        ...
        try:
            primary_result = self.primary.lookup_rules_for_source_fields(
                source_fields=source_fields,
                message_code=message_code,
                target_protocol_type=target_protocol_type,
                target_message_code=target_message_code,
            ...
            if primary_result.rules:
                return primary_result
        except Exception:
            pass

        return self.fallback.lookup_rules_for_source_fields(
            source_fields=source_fields,
            message_code=message_code,
            target_protocol_type=target_protocol_type,
            target_message_code=target_message_code,
