...
PROTOCOL_RULES_CACHE_VERSION = "20260601_no_target_dependency_formulas_v14"
...
_SUPPORTED_TABLE_RULE_EXTENSIONS = {".csv", ".tsv", ".xlsx", ".xls", ".docx"}
...
def _build_inline_table_rule_temp_file(item: Dict[str, Any], temp_dir: Path, index: int) -> Path:
    file_name = str(item.get("file_name") or item.get("filename") or f"table_rule_{index}.csv").strip()
    suffix = Path(file_name).suffix.lower() or ".csv"
    if suffix not in {".csv", ".tsv", ".xlsx", ".xls", ".docx"}:
        raise ValueError(f"暂不支持的表格文件类型: {file_name}")
    ...
    if item.get("content_base64") is not None:
        try:
            raw_bytes = base64.b64decode(str(item.get("content_base64") or "").encode("utf-8"), validate=True)
        except (ValueError, binascii.Error) as exc:
            raise ValueError(f"表格文件base64内容非法: {file_name}") from exc
        file_path.write_bytes(raw_bytes)
        return file_path

    if item.get("content") is not None:
        file_path.write_text(str(item.get("content") or ""), encoding="utf-8")
        return file_path

    raise ValueError(f"表格文件缺少content或content_base64: {file_name}")


def _materialize_table_rule_files(data: Dict[str, Any]) -> Tuple[List[str], Optional[tempfile.TemporaryDirectory[str]]]:
    raw_files: List[Any] = []
    for key in _TABLE_RULE_FILE_KEYS:
        value = data.get(key)
        if value is None:
        ...
        if isinstance(item, dict):
            if temp_dir_obj is None:
                temp_dir_obj = tempfile.TemporaryDirectory(
                    prefix="interface7_table_rules_",
                    dir=str(PROJECT_DIR / "tmp"),
                )
            resolved_files.append(
                str(_build_inline_table_rule_temp_file(item, Path(temp_dir_obj.name), index).resolve())
            )
            continue
        text = str(item or "").strip()
        if text:
            resolved_files.append(text)
    return resolved_files, temp_dir_obj


def _save_uploaded_table_rule_file(uploaded_file) -> Path:
    file_name = str(getattr(uploaded_file, "filename", "") or "").strip()
    if not file_name:
        raise ValueError("缺少上传文件名")
    suffix = Path(file_name).suffix.lower()
    if suffix not in _SUPPORTED_TABLE_RULE_EXTENSIONS:
        raise ValueError(
            f"不支持的表格文件类型: {suffix or file_name}，支持类型: {', '.join(sorted(_SUPPORTED_TABLE_RULE_EXTENSIONS))}"
        )
    ...
    upload_dir = PROJECT_DIR / "tmp" / "uploaded_table_rules"
    upload_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S%f")
    safe_stem = re.sub(r"[^A-Za-z0-9._-]+", "_", Path(file_name).stem).strip("._") or "table_rule"
    saved_path = upload_dir / f"{timestamp}_{safe_stem}{suffix}"
...
def _load_cached_protocol_rules_response(
    ...
    if not rules_file.exists() or not yaml_file.exists():
        return None
    try:
        return _normalize_cached_protocol_rules_response(
            response_payload,
            source_protocol_dir,
            target_protocol_dir,
        )
...
def _normalize_cached_protocol_rules_response(
    response_payload: Dict[str, Any],
    source_protocol_dir: Any,
    target_protocol_dir: Any,
) -> Dict[str, Any]:
    ...
    if isinstance(summary, dict):
        normalized_payload["summary"] = _augment_interface7_summary(summary)

    kg_writeback_payload = normalized_payload.get("kg_writeback_payload")
    if not isinstance(kg_writeback_payload, dict):
        return normalized_payload

    normalized_writeback_payload = dict(kg_writeback_payload)
    normalized_rules = [
        _plainify_writeback_rule_fields(rule_item)
        if isinstance(rule_item, dict)
        else rule_item
        for rule_item in (normalized_writeback_payload.get("rules") or [])
    ]
    normalized_rules = _filter_display_writeback_rules(normalized_rules)
    for rule_item in normalized_rules:
        if isinstance(rule_item, dict):
            rule_item.pop("formula_kind", None)
    normalized_writeback_payload["rules"] = normalized_rules
    normalized_payload["kg_writeback_payload"] = normalized_writeback_payload
    return normalized_payload
...
def _augment_interface7_summary(summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized_summary.setdefault("semantic_match_query_count", 0)
    normalized_summary.setdefault("semantic_match_avg_query_time_ms", None)
    normalized_summary.setdefault("semantic_match_time_target_met", None)
    normalized_summary.setdefault("rule_generation_target_count", 0)
    normalized_summary.setdefault("rule_generation_avg_time_ms", None)
    normalized_summary.setdefault("rule_generation_time_target_met", None)
...
def _build_relation_target_field_token(
    ...
    display_path = str(target_path or "").strip()
    if display_path:
        return display_path.split("/")[-1].strip()
    display_field = _strip_protocol_prefixed_field_name(target_field, target_protocol)
    if display_field:
        return display_field.split("/")[-1].strip()
    actual_field = str(target_actual_field or "").strip()
    if actual_field:
        return _strip_protocol_prefixed_field_name(actual_field, target_protocol) or actual_field
    return _normalize_relation_protocol_prefix(target_protocol) or "field"
...
def _build_table_rule_mode_response(data: Dict[str, Any]) -> Dict[str, Any]:
    table_rule_files, temp_dir_obj = _materialize_table_rule_files(data)
    try:
...
def _strip_balanced_outer_parentheses(text: Any) -> str:
...
def _strip_self_referential_target_guard(expression: Any, target_token: Any) -> str:
    ...
    def _true_branch_matches_compared_value(true_branch: str, compared_value: str) -> Optional[str]:
    ...
    ternary_match = re.fullmatch(
        r"\(?\s*(?P<condition>.+?)\s*\?\s*(?P<value>.+?)\s*:\s*(?P<fallback>.+?)\s*\)?",
        normalized_expression,
    )
    if ternary_match:
        condition = _strip_balanced_outer_parentheses(ternary_match.group("condition"))
        value = ternary_match.group("value").strip()
        fallback = _strip_balanced_outer_parentheses(ternary_match.group("fallback"))
        if re.fullmatch(r"0(?:\.0|U|L)?", fallback):
            condition_patterns = [
    ...
    python_conditional_match = re.fullmatch(
        r"\(?\s*(?P<value>.+?)\s+if\s+(?P<condition>.+?)\s+else\s+(?P<fallback>.+?)\s*\)?",
        normalized_expression,
        flags=re.IGNORECASE,
    )
    if python_conditional_match:
        value = python_conditional_match.group("value").strip()
        condition = _strip_balanced_outer_parentheses(python_conditional_match.group("condition"))
        fallback = _strip_balanced_outer_parentheses(python_conditional_match.group("fallback"))
        if re.fullmatch(r"0(?:\.0)?", fallback):
            condition_patterns = [
                re.compile(rf"^(?P<left>.+?)\s*==\s*{escaped_target}$", flags=re.IGNORECASE),
                re.compile(rf"^{escaped_target}\s*==\s*(?P<right>.+?)$", flags=re.IGNORECASE),
            ]
...
def _build_explicit_formula_target_token(
    ...
    display_seed = _build_relation_target_field_token(
...
def _relation_assignment_target_name(node: ast.AST) -> Optional[str]:
    if isinstance(node, ast.Name):
        return _normalize_relation_protocol_prefix(node.id) or _to_formula_token(node.id)
    if isinstance(node, ast.Attribute):
        return _normalize_relation_protocol_prefix(node.attr) or _to_formula_token(node.attr)
    return None
...
def _build_relation_rule_payload(rule: Dict[str, Any], target_protocol: Optional[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "target_actual_field": target_actual_field,
        "target_path": relation_target_field,
...
def _finalize_relation_rule_target_fields(rules: Sequence[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in rules:
        ...
        base_target_var = str(normalized.get("target_var") or normalized.get("target_field") or "").strip()
        if base_target_var:
            occurrence = seen_counts.get(base_target_var, 0) + 1
            seen_counts[base_target_var] = occurrence
            if occurrence > 1:
                normalized_target_var = f"{base_target_var}_{occurrence}"
                normalized["target_var"] = normalized_target_var
...
def _normalize_manual_writeback_rules(
    ...
    for index, item in enumerate(raw_rules):
        ...
        target_field = ""
        if target_path:
            target_field = target_path.split("/")[-1].strip()
        if not target_field:
            target_field = _strip_protocol_prefixed_field_name(raw_target_field, target_protocol_type)
        if not target_field:
            target_field = _strip_protocol_prefixed_field_name(item.get("target_actual_field"), target_protocol_type)
        normalized_source_fields = [
...
def _displayize_rule_records(
    ...
    for item in rule_records:
        ...
        if plain_writeback_fields:
            merged = _plainify_writeback_rule_fields(merged)
        merged_rules.append(merged)
    return merged_rules


def _plainify_writeback_rule_fields(rule: Dict[str, Any]) -> Dict[str, Any]:
    merged = dict(rule)
...
@app.route("/api/knowledge/upload_table_rule", methods=["POST"])
def upload_table_rule():
    """上传表格规则文件并返回服务端可访问路径。"""
    try:
        ...
        saved_path = _save_uploaded_table_rule_file(uploaded_file)
        source_protocol_type = _first_nonempty_text(
            request.form.get("source_protocol_type"),
            request.form.get("protocol_type"),
        )
        source_message_code = _first_nonempty_text(
            request.form.get("source_message_code"),
            request.form.get("message_code"),
        )
        target_protocol_type = _first_nonempty_text(request.form.get("target_protocol_type"))
        target_message_code = _first_nonempty_text(request.form.get("target_message_code"))
        next_body = {
            "input_mode": "table_rule",
            "table_rule_files": [str(saved_path)],
        }
        if source_protocol_type:
            next_body["source_protocol_type"] = source_protocol_type
        if source_message_code:
            next_body["source_message_code"] = source_message_code
        if target_protocol_type:
            next_body["target_protocol_type"] = target_protocol_type
        if target_message_code:
            next_body["target_message_code"] = target_message_code
...
def protocol_generate_rules():
    ...
    try:
        ...
        total_semantic_match_queries = 0
        total_semantic_match_time_ms = 0.0
        total_rule_generation_targets = 0
        total_rule_generation_time_ms = 0.0
        ...
        for candidate in selected_candidates:
            ...
            total_semantic_match_queries += int(result_summary.get("semantic_match_query_count") or 0)
            total_semantic_match_time_ms += float(result_summary.get("semantic_match_time_ms_total") or 0.0)
            total_rule_generation_targets += int(result_summary.get("rule_generation_target_count") or 0)
            total_rule_generation_time_ms += float(result_summary.get("rule_generation_time_ms_total") or 0.0)
        ...
        if total_semantic_match_queries > 0:
            semantic_match_avg_query_time_ms = round(
                total_semantic_match_time_ms / total_semantic_match_queries,
                4,
            )
            semantic_match_time_target_met = semantic_match_avg_query_time_ms <= 20.0
        rule_generation_avg_time_ms = None
        rule_generation_time_target_met = None
        if total_rule_generation_targets > 0:
            rule_generation_avg_time_ms = round(
                total_rule_generation_time_ms / total_rule_generation_targets,
                4,
            )
            rule_generation_time_target_met = rule_generation_avg_time_ms <= 50.0
        response_payload = {
            **saved_rule_paths,
            "relations": relations,
            ...
            "summary": _augment_interface7_summary({
                "semantic_match_query_count": total_semantic_match_queries,
                "semantic_match_avg_query_time_ms": semantic_match_avg_query_time_ms,
                "semantic_match_time_target_met": semantic_match_time_target_met,
                "rule_generation_target_count": total_rule_generation_targets,
                "rule_generation_avg_time_ms": rule_generation_avg_time_ms,
                "rule_generation_time_target_met": rule_generation_time_target_met,
                "knowledge_graph_field_count": total_knowledge_graph_fields,
                "knowledge_graph_avg_rule_time_ms": rule_generation_avg_time_ms,
                "knowledge_graph_rule_time_target_met": rule_generation_time_target_met,
...
def _build_udp_gateway_config() -> Dict[str, Any]:
    ...
    return normalize_udp_gateway_config(
        runner,
        default_route="/api/knowledge/protocol_generate_rules",
        allowed_routes=_UDP_ALLOWED_ROUTES,
        runtime_root=PROJECT_DIR / "runtime",
        app_name="07_protocol_generate_rules",
    )
