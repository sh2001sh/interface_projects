...
def _neo4j_connection_timeout_seconds(neo4j: Dict[str, Any]) -> float:
    raw = (
        neo4j.get("connection_timeout_seconds")
        or neo4j.get("connection-timeout-seconds")
        or neo4j.get("timeout_seconds")
        or 5.0
    )
    return float(raw)


def _neo4j_max_connection_pool_size(neo4j: Dict[str, Any]) -> int:
    raw = (
        neo4j.get("max_connection_pool_size")
        or neo4j.get("max-connection-pool-size")
        or 50
    )
    return int(raw)
...
class RuntimeConfig:
    ...
    def get_server(self) -> Dict[str, Any]:
        ...
        merged = {"host": "0.0.0.0", "port": 6107, "debug": True, "threaded": False}
        if isinstance(server, dict):
            merged.update(server)
        merged["port"] = int(merged.get("port") or 6107)
    ...
    def export_env_defaults(self) -> Dict[str, str]:
        ...
        neo4j_timeout = _neo4j_connection_timeout_seconds(neo4j)
        neo4j_pool_size = _neo4j_max_connection_pool_size(neo4j)
        ...
        env: Dict[str, str] = {
            ...
            "PROTOCOL_CONVERSION_NEO4J_TIMEOUT_SECONDS": str(neo4j_timeout),
            "PROTOCOL_CONVERSION_NEO4J_CONNECTION_TIMEOUT_SECONDS": str(neo4j_timeout),
            "PROTOCOL_CONVERSION_NEO4J_MAX_CONNECTION_POOL_SIZE": str(neo4j_pool_size),
            "PROTOCOL_CONVERSION_NEO4J_AUTO_INIT": _normalize_bool(neo4j["auto_init"]),
            "PROTOCOL_CONVERSION_NEO4J_SCHEMA_MODE": str(neo4j.get("schema_mode") or "auto"),
            ...
            "NEO4J_CONNECTION_TIMEOUT_SECONDS": str(neo4j_timeout),
            "NEO4J_MAX_CONNECTION_POOL_SIZE": str(neo4j_pool_size),
