...
DEFAULT_CONFIG: Dict[str, Any] = {
    "version": 1,
    "server": {"host": "0.0.0.0", "port": 6102, "debug": True, "threaded": False},
    "processing": {
        "docling_worker_timeout_seconds": 1800,
        "huge_pdf_page_threshold": 1000,
        "huge_pdf_docling_enhance_max_pages": 0,
        "huge_pdf_docling_enhance_batch_size": 8,
    },
    "llm": {
...
class RuntimeConfig:
    ...
    def get_server(self) -> Dict[str, Any]:
        ...
        merged = {"host": "0.0.0.0", "port": 6102, "debug": True, "threaded": False}
        if isinstance(server, dict):
            merged.update(server)
        merged["port"] = int(merged.get("port") or 6102)
    ...
    def export_env_defaults(self) -> Dict[str, str]:
        ...
        processing = self.data.get("processing") if isinstance(self.data.get("processing"), dict) else {}
        ...
        env: Dict[str, str] = {
            ...
            "DOCLING_WORKER_TIMEOUT_SECONDS": str(processing.get("docling_worker_timeout_seconds") or 1800),
            "DOCLING_HUGE_PDF_PAGE_THRESHOLD": str(processing.get("huge_pdf_page_threshold") or 1000),
            "DOCLING_HUGE_PDF_ENHANCE_MAX_PAGES": str(processing.get("huge_pdf_docling_enhance_max_pages", 0)),
            "DOCLING_HUGE_PDF_ENHANCE_BATCH_SIZE": str(processing.get("huge_pdf_docling_enhance_batch_size") or 8),
        }
