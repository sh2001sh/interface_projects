...
XML_LIKE_PATTERN = re.compile(r"<[^>]+>")
LLM_STRONG_SIGNAL_PATTERNS = (
    re.compile(r"J\d+\.\d+[A-Z]?\d*", re.IGNORECASE),
    re.compile(r"\b(?:bit|bits|位|resolution|分辨率|formula|conversion|convert|mapping)\b", re.IGNORECASE),
    re.compile(r"\b[A-Z][A-Z0-9_]{2,}\b"),
)
LLM_TABLE_HINT_PATTERN = re.compile(r"[|\t]")
...
def _count_llm_signal_hits(content: str) -> int:
    return sum(1 for pattern in LLM_STRONG_SIGNAL_PATTERNS if pattern.search(content))


def _should_use_llm_postprocess(content: str, block_type: str, rule_fields: List[Dict[str, Any]]) -> bool:
    normalized = (content or "").strip()
    if rule_fields:
        return False
    if len(normalized) < 120 or len(normalized) > 2200:
        return False
    if XML_LIKE_PATTERN.search(normalized) and normalized.count("<") >= 3:
        return False

    lines = [line.strip() for line in normalized.splitlines() if line.strip()]
    if len(lines) > 60 and block_type != "table":
        return False

    signal_hits = _count_llm_signal_hits(normalized)
    has_table_layout = bool(LLM_TABLE_HINT_PATTERN.search(normalized))
    if block_type == "table":
        return signal_hits >= 1 and (has_table_layout or len(lines) >= 3)
    return signal_hits >= 2 and has_table_layout
...
def enrich_protocol_metadata(block: Dict[str, Any], enable_llm_postprocess: bool) -> Dict[str, Any]:
    ...
    should_use_llm = enable_llm_postprocess and _should_use_llm_postprocess(
        content,
