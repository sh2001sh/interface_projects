...
class Block:
    ...
    def _resolved_page_range(self) -> List[int]:
        if self.page_range:
            normalized: List[int] = []
            for value in self.page_range:
                try:
                    page_num = int(value)
        ...
        merged_pages = metadata.get("merged_pages") if isinstance(metadata.get("merged_pages"), list) else []
        normalized: List[int] = []
        ...
        try:
            ...
            end_page = 0
        if self.page_num > 0 and end_page > 0:
            start_page, stop_page = sorted((self.page_num, end_page))
            return list(range(start_page, stop_page + 1))
        if self.page_num > 0:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "page_range": self._resolved_page_range(),
